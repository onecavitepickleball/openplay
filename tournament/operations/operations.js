...
const names = (state, category, code) => { const pair = state.pairs?.[`${category}|${code}`] || {}; return [pair.player1, pair.player2].filter(Boolean).join(' / ') || code || 'TBD'; };
const pairClub = (config, code) => config.clubs.find(club => String(code || '').startsWith(club.pairPrefix)) || {};
...
function renderRoster() {
  const rows = (state.matches || []).flatMap(match => [['a','ocpc'],['b','rebels']].flatMap(([side,clubId]) => [0,1].map(index => { const code = match[side], pair = state.pairs?.[`${match.category}|${code}`] || {}; const name = pair[index ? 'player2' : 'player1']; if (!name) return null; return { category:match.category, code, clubId, name, checked:checkedIn(match.category,code,clubId,index) }; }))).filter(Boolean);
...
function scoreSheet(match) {
  ...
  return `<article class="score-sheet"><div class="sheet-meta"><span>${escapeHtml(stage)} · ${escapeHtml(match.category)}</span><span>Court ${escapeHtml(match.court || 'TBD')} · ${escapeHtml(timeLabel(match.startMinutes))}</span></div><div class="versus"><div class="team-box">${escapeHtml(names(state,match.category,match.a))}<br><small>${escapeHtml(pairClub(config,match.a).short || '')} · ${escapeHtml(match.a)}</small></div><div class="score-box">VS</div><div class="team-box">${escapeHtml(names(state,match.category,match.b))}<br><small>${escapeHtml(pairClub(config,match.b).short || '')} · ${escapeHtml(match.b)}</small></div></div><div class="score-lines"><div class="line-block">Final score: ______<br>Timeouts: ______<br>Notes: __________________<br>________________________</div><div class="line-block">Winner: __________________<br>Referee: _________________<br>Start: ______ End: ______<br>________________________</div></div><div class="signature"><span>Club A confirmation</span><span>Club B confirmation</span></div></article>`;
}
