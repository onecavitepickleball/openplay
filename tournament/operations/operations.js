...
const names = (state, category, code) => { const pair = state.pairs?.[`${category}|${code}`] || state.pairs?.[code] || {},players=[pair.player1,pair.player2].filter(Boolean);return pair.name||pair.names||(pair.teamName?`${pair.teamName}${players.length?` (${players.join(' & ')})`:''}`:players.join(' / '))||'Players pending'; };
...
function renderSchedules() {
  ...
  $('#courtSchedules').innerHTML = Array.from({ length:courts }, (_, index) => index + 1).map(court => {
    ...
    return `<article class="court-card"><h3>Court ${court}</h3>${matches.length ? `<table><thead><tr><th>Time</th><th>Match</th><th>Stage</th></tr></thead><tbody>${matches.map(match => `<tr><td>${escapeHtml(timeLabel(match.startMinutes))}</td><td><span class="pair">${escapeHtml(names(state,match.category,match.a))}</span><small>${escapeHtml(pairClub(config,match.a,match.category,state).name || pairClub(config,match.a,match.category,state).short || 'Unaffiliated')}</small><br>vs<br><span class="pair">${escapeHtml(names(state,match.category,match.b))}</span><small>${escapeHtml(pairClub(config,match.b,match.category,state).name || pairClub(config,match.b,match.category,state).short || 'Unaffiliated')}</small><br><small>${escapeHtml(match.category)}</small></td><td class="match-stage">${escapeHtml(match.stage === 'medal' ? (match.medalKey === 'bronze' ? 'Bronze' : 'Gold / Silver') : 'Round robin')}</td></tr>`).join('')}</tbody></table>` : '<div class="empty">No matches assigned.</div>'}</article>`;
  }).join('');
}
