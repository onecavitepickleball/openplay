...
function scoreSheet(match) {
  ...
  return `<article class="score-sheet"><div class="sheet-meta"><span>${escapeHtml(stage)} · ${escapeHtml(match.category)}</span><span>Court ${escapeHtml(match.court || 'TBD')} · ${escapeHtml(timeLabel(match.startMinutes))}</span></div><div class="versus"><div class="team-box">${escapeHtml(names(state,match.category,match.a))}<br><small>${escapeHtml(pairClub(config,match.a,match.category,state).short || '')} · ${escapeHtml(match.a)}</small></div><div class="score-box">VS</div><div class="team-box">${escapeHtml(names(state,match.category,match.b))}<br><small>${escapeHtml(pairClub(config,match.b,match.category,state).short || '')} · ${escapeHtml(match.b)}</small></div></div><div class="score-lines"><div class="line-block">Final score: ______<br>Timeouts: ______<br>Notes: __________________<br>________________________</div><div class="line-block">Winner: __________________<br>Referee: _________________<br>Start: ______ End: ______<br>________________________</div></div><div class="signature"><span>Side A confirmation</span><span>Side B confirmation</span></div></article>`;
}
