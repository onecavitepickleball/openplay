...
(() => {
  ...
  function queuePublicProjection(){if(!state.publicShare?.token||demoMode||drawTestMode)return;clearTimeout(publicWriteTimer);publicWriteTimer=setTimeout(()=>publishPublicView(state.publicShare.token,publicProjection()).catch(()=>{}),250);}
  ...
  function bracketMatch(category, key, label, time, court, a, b, scores) {
    ...
    const side = (code, score, scoreKey, seedKey) => `<label class="bracket-side ${result.winner === code && code ? 'winner' : ''}"><span><b><input class="seed-input" type="text" data-medal-category="${esc(category)}" data-medal-match="${key}" data-medal-side="${seedKey}" value="${esc(code || '')}" placeholder="Pair code"></b><small>${code ? esc(pairNames(category, code)) : 'Enter pair code manually'}</small></span><input type="number" min="0" max="19" data-medal-category="${esc(category)}" data-medal-match="${key}" data-medal-side="${scoreKey}" value="${score}"></label>`;
  ...
  function renderMedals() {
    ...
    $$('[data-medal-category]').forEach(input => input.onchange = () => { const match = state.medals[input.dataset.medalCategory][input.dataset.medalMatch]; match[input.dataset.medalSide] = input.dataset.medalSide === 'a' || input.dataset.medalSide === 'b' ? input.value.trim() : input.value === '' ? '' : Number(input.value); saveState(); renderMedals(); });
  }
