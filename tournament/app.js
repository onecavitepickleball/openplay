...
(() => {
  ...
  $('#assignRefereeBtn').onclick = () => { const email = $('#refereeEmail').value.trim().toLowerCase(), matchId = $('#refereeMatch').value; if (!email || !matchId) return toast('Choose a match and an available referee.'); state.refereeAssignments[matchId] = email; $('#refereeEmail').value = ''; saveState(); renderSettings(); renderCourts(); toast('Referee assigned.'); };
