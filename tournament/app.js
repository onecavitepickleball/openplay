...
(() => {
  ...
  function showDrawWheel(category) {
    ...
    const code = categoryDraft.ocpc[categoryDraft.revealed], colors = ['#4fb6ff','#b6ff3c','#ffc94d','#b783ff','#ff6c63','#54dab1','#ff8bc3'];
    let wheelOrder = [...categoryDraft.rebels];
    ...
    $('#drawWheelOverlay')?.remove(); document.body.insertAdjacentHTML('beforeend', `<div class="draw-wheel-overlay" id="drawWheelOverlay"><section><button class="modal-close contrast-close" id="closeDrawWheel">×</button><span class="section-label">${esc(category)} · ${esc(displayPair(category,code))}</span><h2>Spin for excluded opponent</h2><p>${esc(pairNames(category,code))} will not be scheduled against the valid pair selected when the wheel stops.</p><div class="draw-wheel-wrap" id="drawWheelWrap"><i class="wheel-pointer"></i>${wheelMarkup()}</div><div class="wheel-controls"><button class="btn btn-quiet" id="shuffleDrawWheel">Shuffle names</button><button class="btn btn-primary wheel-spin-button" id="spinDrawWheel">Spin the wheel · 15 seconds</button></div><div class="wheel-result" id="wheelResult" hidden></div></section></div>`);
    ...
    const omissionsFor = (order,pairCode) => { const played = categoryDrawMatches(category,{...categoryDraft,rebels:order}).filter(match=>match.a===pairCode).map(match=>match.b); return order.filter(other=>!played.includes(other)); };
    const sameSet = (a,b) => a.length===b.length && a.every(value=>b.includes(value));
    const overlay=$('#drawWheelOverlay'); $('#closeDrawWheel').onclick=()=>overlay.remove(); $('#shuffleDrawWheel').onclick=()=>{ shuffleOrder(); $('#drawWheel').outerHTML=wheelMarkup(); toast('Names shuffled. The stop will still be generated only when Spin is pressed.'); }; $('#spinDrawWheel').onclick=()=>{ const prior=categoryDraft.wheelResults||{}, valid=permutations(categoryDraft.rebels).filter(order=>Object.entries(prior).every(([pairCode,result])=>sameSet(omissionsFor(order,pairCode),result))), random=crypto.getRandomValues(new Uint32Array(1))[0], selectedOrder=valid[random%valid.length], selectedOmissions=omissionsFor(selectedOrder,code), selected=selectedOmissions[0], selectedIndex=wheelOrder.indexOf(selected), segment=360/wheelOrder.length, finalDegrees=7200+(360-(selectedIndex*segment+segment/2)); categoryDraft.rebels=selectedOrder; categoryDraft.wheelResults={...prior,[code]:selectedOmissions}; state.drawCeremonyDraft.log.push({action:'wheel-spun',category,pair:code,selectedOmissions:[...selectedOmissions],validOutcomes:valid.length,at:new Date().toISOString()}); saveState(); const wheel=$('#drawWheel'); wheel.style.setProperty('--spin-degrees',`${finalDegrees}deg`); wheel.classList.add('spinning'); $('#spinDrawWheel').disabled=true; $('#shuffleDrawWheel').disabled=true; setTimeout(()=>{ $('#wheelResult').innerHTML=`<span>Wheel selected · Not scheduled against</span>${selectedOmissions.map(pairCode=>`<b>${esc(pairNames(category,pairCode))}<small>${esc(displayPair(category,pairCode))}</small></b>`).join('')}<button class="btn btn-primary" id="confirmWheelResult">Record this wheel result and continue</button>`; $('#wheelResult').hidden=false; $('#confirmWheelResult').onclick=()=>{overlay.remove();revealNextDraw(category,true)}; },15100); };
  }
  ...
  function startCloud() {
    ...
    watchAuth(async user => {
      ...
      watchControl(incoming => { if (!incoming) { if (!drawTestMode) publishControl(state).catch(() => showCloudGate('Your account cannot initialize this event.')); return; } const formatChanged = incoming.schedulerVersion !== 7, scheduleChanged = Number(incoming.scheduleBaselineStartMinutes) !== config.event.roundRobinStartMinutes, testDraft = drawTestMode ? state.drawCeremonyDraft : null; cloudApplying = true; const localCheckins = state.checkins || {}, normalized = normalizeState(incoming); state = { ...normalized, checkins: { ...normalized.checkins, ...localCheckins }, liveScoring: formatChanged ? {} : state.liveScoring || {} }; if (drawTestMode) { state.drawCeremonyDraft = testDraft; state.opponentDrawHistory = []; } controlReady = true; hydrateRegisteredTeams(false); localStorage.setItem(storageKey, JSON.stringify(state)); renderAll(); if (drawTestMode) showView('draw'); cloudApplying = false; if (!drawTestMode && formatChanged) clearMatches().then(() => publishControl(state)).then(() => toast('95-match format and normalized standings applied. Previous test match data cleared.')).catch(() => toast('Format updated locally; cloud cleanup needs another try.')); else if (!drawTestMode && scheduleChanged) publishControl(state).then(() => toast('Tournament start moved to 10:30 AM.')).catch(() => toast('Schedule updated locally; cloud sync needs another try.')); else if (!drawTestMode && !refereeBoardPublished) { refereeBoardPublished = true; publishControl(state).catch(() => { refereeBoardPublished = false; }); } }, () => showCloudGate('Your account does not have Match Control access.'));
