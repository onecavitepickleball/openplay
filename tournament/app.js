...
(() => {
  ...
  function showRefereeAssignment(matchId, courtNo) {
    ...
    $('#saveRefereeAssignment').onclick = () => { const email = $('#assignmentRefereeEmail').value.trim().toLowerCase(); if (!email) return toast('Choose an available referee.'); state.refereeAssignments[matchId] = email; saveState(); close(); renderAll(); toast(`${refereeName(email)} assigned to ${match.id}.`); };
