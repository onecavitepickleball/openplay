...
(() => {
  ...
  function matchupKey(match) { return `${config.categories.indexOf(match.category)}|${match.a}|${match.b}`; }
  function stableMatchJitter(match) { let hash = 2166136261; for (const char of matchupKey(match)) { hash ^= char.charCodeAt(0); hash = Math.imul(hash, 16777619); } return (hash >>> 0) / 4294967296 * 3; }
  ...
  function timeInput(totalMinutes) { const value = Math.max(0, Math.min(1439, Number(totalMinutes) || 0)); return `${String(Math.floor(value / 60)).padStart(2, '0')}:${String(value % 60).padStart(2, '0')}`; }
  function inputMinutes(value) { const [hours, minutes] = String(value || '00:00').split(':').map(Number); return Math.max(0, Math.min(1439, (hours || 0) * 60 + (minutes || 0))); }
  ...
  function defaultFormatSettings() { return { mode: 'capped', matchCap: 5 }; }
  function defaultScheduleSettings() { return { categoryFlow: 'alternating', priority: 'balanced', minRestSlots: 1, targetMaxGapSlots: 4 }; }
  function defaultQualificationSettings() { return { order: ['wins', 'pointsFor', 'pointsAgainst', 'pointDifferential', 'headToHead'] }; }
  const qualificationLabels = { wins: 'Number of Wins', pointsFor: 'Points For', pointsAgainst: 'Points Against', pointDifferential: 'Point Differential', headToHead: 'Head to Head' };
  function formatSettings(value = state?.formatSettings) { return { ...defaultFormatSettings(), ...(value || {}), matchCap: Math.max(1, Number(value?.matchCap) || 5) }; }
  function scheduleSettings(value = state?.scheduleSettings) { return { ...defaultScheduleSettings(), ...(value || {}), minRestSlots: Math.max(0, Number(value?.minRestSlots) || 0), targetMaxGapSlots: Math.max(1, Number(value?.targetMaxGapSlots) || 4) }; }
  function qualificationSettings(value = state?.qualificationSettings) { const dualMeet = (value?.format || config.competitionType || 'dual-meet') === 'dual-meet', allowed = Object.keys(qualificationLabels).filter(item => !dualMeet || item !== 'headToHead'), supplied = Array.isArray(value?.order) ? value.order.filter((item, index, list) => allowed.includes(item) && list.indexOf(item) === index) : []; return { format: dualMeet ? 'dual-meet' : 'standard', order: [...supplied, ...allowed.filter(item => !supplied.includes(item))] }; }
  ...
  function categoryMatchCount(category, counts = state?.pairCounts || defaultPairCounts(), settings = formatSettings()) { const ocpc = pairCount(category, 'ocpc', counts), rebels = pairCount(category, 'rebels', counts), rules = formatSettings(settings); return rules.mode === 'full' ? ocpc * rebels : Math.max(ocpc, rebels) * Math.min(rules.matchCap, Math.min(ocpc, rebels)); }
  function totalMatchCount(counts = state?.pairCounts || defaultPairCounts(), settings = formatSettings()) { return config.categories.reduce((total, category) => total + categoryMatchCount(category, counts, settings), 0); }
  function categoryNeedsDraw(category, counts = state?.pairCounts || defaultPairCounts(), settings = formatSettings()) { const rules = formatSettings(settings); return rules.mode !== 'full' && Math.min(rules.matchCap, Math.min(pairCount(category, 'ocpc', counts), pairCount(category, 'rebels', counts))) < Math.max(pairCount(category, 'ocpc', counts), pairCount(category, 'rebels', counts)); }
  function drawSuggested(counts = state?.pairCounts || defaultPairCounts(), settings = formatSettings()) { return config.categories.some(category => categoryNeedsDraw(category, counts, settings)); }
  ...
  function categoryDrawMatches(category, order, counts = state?.pairCounts || defaultPairCounts(), settings = formatSettings()) {
    const ocpcCount = pairCount(category, 'ocpc', counts), rebelsCount = pairCount(category, 'rebels', counts), matches = [], rules = formatSettings(settings), opponentLimit = Math.min(rules.matchCap, Math.min(ocpcCount, rebelsCount));
    if (rules.mode === 'full') { order.ocpc.forEach(a => order.rebels.forEach(b => matches.push({ category, round: 0, a, b }))); return matches; }
  ...
  function scheduleMatchList(sourceMatches, settings = scheduleSettings()) {
    const rules = scheduleSettings(settings), total = sourceMatches.length, fullWaves = Math.floor(total / config.event.courts), remainder = total % config.event.courts;
    const capacities = [...Array(fullWaves).fill(config.event.courts), ...(remainder ? [remainder] : [])];
    const remaining = sourceMatches.map(match => ({ ...match })).sort((a, b) => matchupKey(a).localeCompare(matchupKey(b))), waves = [], lastPlayed = new Map(), appearances = new Map(); let previous = new Set();
    capacities.forEach((capacity, waveIndex) => { const selected = [], used = new Set(); while (selected.length < capacity && remaining.length) { const courtIndex = selected.length, targetCategory = config.categories[(waveIndex + courtIndex) % config.categories.length], sameWaveSafe = remaining.filter(match => !match.players.some(player => used.has(player))), rested = sameWaveSafe.filter(match => !match.players.some(player => previous.has(player))), pool = rested.length ? rested : sameWaveSafe.length ? sameWaveSafe : remaining, alternating = rules.categoryFlow === 'alternating' ? pool.filter(match => match.category === targetCategory) : [], candidates = alternating.length ? alternating : pool; candidates.sort((a, b) => { const score = match => { const waits = match.players.map(player => lastPlayed.has(player) ? waveIndex - lastPlayed.get(player) : rules.targetMaxGapSlots), wait = Math.min(...waits), longest = Math.max(...waits), load = match.players.reduce((sum, player) => sum + (appearances.get(player) || 0), 0), groupedTarget = config.categories[Math.min(config.categories.length - 1, Math.floor(waveIndex / Math.max(1, Math.ceil(capacities.length / config.categories.length))))], categoryBonus = rules.categoryFlow === 'grouped' && match.category === groupedTarget ? 130 : rules.categoryFlow === 'alternating' && match.category === targetCategory ? 180 : 0, b2bPenalty = wait <= rules.minRestSlots ? (rules.priority === 'utilization' ? 180 : 900) : 0, overdueBonus = Math.max(0, longest - rules.targetMaxGapSlots) * (rules.priority === 'compact' ? 220 : 110), restWeight = rules.priority === 'rest' ? 85 : rules.priority === 'compact' ? 45 : 60; return categoryBonus + wait * restWeight + overdueBonus - load * 10 - b2bPenalty + stableMatchJitter(match); }; return score(b) - score(a) || matchupKey(a).localeCompare(matchupKey(b)); }); const match = candidates[0]; selected.push(match); match.players.forEach(player => used.add(player)); remaining.splice(remaining.indexOf(match), 1); } waves.push(selected); previous = new Set(selected.flatMap(match => match.players)); selected.forEach(match => match.players.forEach(player => { lastPlayed.set(player, waveIndex); appearances.set(player, (appearances.get(player) || 0) + 1); })); });
    const waveAccepts = (wave, candidate, replacing = -1) => !wave.some((match, index) => index !== replacing && match.players.some(player => candidate.players.includes(player)));
    for (let waveIndex = 0; waveIndex < waves.length; waveIndex++) {
      let repaired = true;
      while (repaired) {
        repaired = false; const seen = new Set(); let conflictIndex = -1;
        waves[waveIndex].forEach((match, index) => { if (conflictIndex >= 0) return; if (match.players.some(player => seen.has(player))) conflictIndex = index; else match.players.forEach(player => seen.add(player)); });
        if (conflictIndex < 0) break;
        const conflict = waves[waveIndex][conflictIndex], candidates = [];
        const adjacentConflict = (match, targetWave) => [targetWave-1,targetWave+1].filter(index => waves[index]).some(index => waves[index].some(other => match.players.some(player => other.players.includes(player))));
        for (let otherWave = 0; otherWave < waves.length; otherWave++) if (otherWave !== waveIndex && (waveIndex === 0 || otherWave !== 0)) for (let otherIndex = 0; otherIndex < waves[otherWave].length; otherIndex++) { const candidate = waves[otherWave][otherIndex]; if (waveAccepts(waves[waveIndex], candidate, conflictIndex) && waveAccepts(waves[otherWave], conflict, otherIndex)) { const categoryPenalty = candidate.category === conflict.category ? 0 : 10, adjacencyPenalty = (adjacentConflict(candidate, waveIndex) ? 50 : 0) + (adjacentConflict(conflict, otherWave) ? 50 : 0); candidates.push({ otherWave, otherIndex, penalty: categoryPenalty + adjacencyPenalty }); } }
        candidates.sort((a,b)=>a.penalty-b.penalty); const swap = candidates[0]; if (!swap) break; waves[waveIndex][conflictIndex] = waves[swap.otherWave][swap.otherIndex]; waves[swap.otherWave][swap.otherIndex] = conflict; repaired = true;
      }
    }
    const backToBackCount = () => { const appearances = new Map(); waves.forEach((wave,index) => wave.forEach(match => match.players.forEach(player => { if (!appearances.has(player)) appearances.set(player,[]); appearances.get(player).push(index); }))); let count=0; appearances.forEach(indexes => { indexes.sort((a,b)=>a-b); for(let index=1;index<indexes.length;index++) if(indexes[index]-indexes[index-1]===1) count++; }); return count; };
    for (let pass=0; pass<12; pass++) { const baseline=backToBackCount(); if (!baseline) break; let best=null; outer: for(let waveIndex=1;waveIndex<waves.length;waveIndex++) for(let matchIndex=0;matchIndex<waves[waveIndex].length;matchIndex++) for(let otherWave=1;otherWave<waves.length;otherWave++) if(otherWave!==waveIndex) for(let otherIndex=0;otherIndex<waves[otherWave].length;otherIndex++) { const first=waves[waveIndex][matchIndex],second=waves[otherWave][otherIndex]; if(first.category!==second.category || !waveAccepts(waves[waveIndex],second,matchIndex) || !waveAccepts(waves[otherWave],first,otherIndex)) continue; waves[waveIndex][matchIndex]=second; waves[otherWave][otherIndex]=first; const score=backToBackCount(); waves[waveIndex][matchIndex]=first; waves[otherWave][otherIndex]=second; if(score<baseline){best={waveIndex,matchIndex,otherWave,otherIndex};break outer;} } if(!best) break; [waves[best.waveIndex][best.matchIndex],waves[best.otherWave][best.otherIndex]]=[waves[best.otherWave][best.otherIndex],waves[best.waveIndex][best.matchIndex]]; }
    return waves.flatMap((wave, waveIndex) => wave.map((match, courtIndex) => { const start = config.event.roundRobinStartMinutes + waveIndex * config.event.slotMinutes; return { ...match, wave: waveIndex + 1, court: courtIndex + 1, startMinutes: start, time: `${timeLabel(start)}-${timeLabel(start + config.event.slotMinutes)}` }; }));
  }
  function generateSchedule(pairCounts, opponentDraw, format = formatSettings(), scheduling = scheduleSettings()) {
    let number = 0; return scheduleMatchList(generateMatches(pairCounts, opponentDraw, format), scheduling).map(match => ({ ...match, id: `RR-${String(++number).padStart(3, '0')}` }));
  }
  ...
  function loadState() {
    try {
      ...
      if (saved?.pairCounts && saved?.matches?.length === totalMatchCount(saved.pairCounts, saved.formatSettings) && saved.pairs && saved.scores) {
  ...
  function saveState(operational = false) {
    ...
    if (!operational && !demoMode && !drawTestMode && cloudUser && !cloudApplying) {
      ...
      cloudSaveTimer = setTimeout(() => {
        controlWriteChain = controlWriteChain.catch(() => {}).then(() => publishControl(pendingState))
  ...
  function queuePublicProjection(){
    ...
    const sequence=++publicWriteSequence;
      publicWriteTimer=setTimeout(()=>{
        if(sequence!==publicWriteSequence||!state.publicShare?.token)return;
        const token=state.publicShare.token,payload=publicProjection();
      publicWriteChain=publicWriteChain.catch(()=>{}).then(()=>{
        if(sequence!==publicWriteSequence||!state.publicShare?.token)return;
        return publishPublicView(token,payload);
      }).catch(error=>{
        console.warn('Live public standings sync failed',error);
        if(sequence===publicWriteSequence&&navigator.onLine!==false) setTimeout(()=>queuePublicProjection(),1000);
      });
    },0);
  }
  function syncMatch(matchId) {
    ...
    const chain=(matchWriteChains.get(matchId)||Promise.resolve()).catch(()=>{}).then(()=>publishMatch(matchId, live, score)); matchWriteChains.set(matchId,chain);
    return chain
      .catch(error => { toast('Live match sync failed. Check Firebase access.'); throw error; })
  ...
  function pairNames(category, code) { if (!code) return 'TBA'; const p = pairData(category, code), players=[p.player1,p.player2].filter(Boolean); return p.name || (p.teamName ? `${p.teamName}${players.length?` (${players.join(' & ')})`:''}` : players.join(' / ')) || 'Players not assigned'; }
  ...
  function refereeName(email) { const person = refereeDirectory.find(item => String(item.email).toLowerCase() === String(email).toLowerCase()); return person ? [person.firstName, person.lastName].filter(Boolean).join(' ') || person.email : email; }
  function categoryPrefix(category) { if (category === 'Novice') return 'NOV'; if (category === 'Low Intermediate') return 'LOW'; if (category === 'High Intermediate') return 'HIGH'; return String(category).replace(/[^a-z0-9 ]/gi,'').split(/\s+/).filter(Boolean).map(word=>word[0]).join('').slice(0,4).toUpperCase() || 'CAT'; }
  ...
  function hydrateRegisteredTeams(shouldSave = true) {
    ...
    registrations.filter(item => item.status === 'confirmed' && item.category && item.pairCode && item.players?.length >= 2).forEach(item => { const key = `${item.category}|${item.pairCode}`,players=[item.players[0].fullName || '',item.players[1].fullName || ''].filter(Boolean),teamName=String(item.teamName||'').trim(); if (pairs[key]) pairs[key] = { player1: players[0] || '', player2: players[1] || '', teamName, name:teamName?`${teamName} (${players.join(' & ')})`:players.join(' / '), club:item.club || '', clubName:item.clubName || '', registrationId: item.id }; });
  ...
  function setBrandContent() {
    ...
    if ($('#scheduleClubA')) $('#scheduleClubA').textContent = `${clubLabel('ocpc',true)} pair`;
    if ($('#scheduleClubB')) $('#scheduleClubB').textContent = `${clubLabel('rebels',true)} pair`;
  ...
  function miniMatchMarkup(match) {
    ...
    return `<div class="mini-match"><span class="court-pill">Next</span><div class="pair"><b>${displayPair(match.category, match.a)}</b><small>${esc(pairNames(match.category, match.a))} · ${esc(clubLabel('ocpc'))}</small></div><span class="vs">vs</span><div class="pair"><b>${displayPair(match.category, match.b)}</b><small>${esc(pairNames(match.category, match.b))} · ${esc(clubLabel('rebels'))}</small></div><button class="score-chip ${done ? 'done' : ''}" data-score-id="${match.id}">${done ? `${score.a}-${score.b}` : 'Add score'}</button></div>`;
  }
  function renderOverview() {
    ...
    const readiness = Object.keys(state.courts).map(no => { const id = nextForCourt(Number(no)), match = state.matches.find(item => item.id === id); if (!match) return `<div class="leader-item"><div class="leader-rank">${no}</div><div><b>Court ${no}</b><small>No match waiting</small></div></div>`; const missing = []; if (checkinWarning(match)) missing.push('check-in'); if (!state.refereeAssignments?.[id]) missing.push('referee'); if (activePlayerCourtConflicts(id).length) missing.push('player conflict'); return `<div class="leader-item"><div class="leader-rank">${no}</div><div><b>${esc(match.id)} · ${missing.length ? 'Needs attention' : 'Court-ready'}</b><small>${missing.length ? esc(missing.join(' · ')) : 'Checked in, no conflict, ready to call'}</small></div></div>`; }).join('');
    $('#courtReadiness').innerHTML = readiness || '<p class="empty">Court readiness will appear when matches are scheduled.</p>';
    const utilization = Object.keys(state.courts).map(no => { const ids = state.matches.filter(match => Number(match.court) === Number(no)).map(match => match.id), done = ids.filter(isComplete).length, active = state.courts[no]?.matchId; return `<div class="leader-item"><div class="leader-rank">${no}</div><div><b>Court ${no} · ${done}/${ids.length} complete</b><small>${active ? `Live: ${esc(active)}` : `Idle · ${Math.max(0, ids.length-done)} remaining`}</small></div></div>`; }).join('');
    $('#courtUtilization').innerHTML = utilization || '<p class="empty">Court utilization will appear when a schedule is generated.</p>';
    bindScoreButtons();
  }
  ...
  function checkinRecord(category, code, clubId, playerIndex) { return Object.values(state.checkins || {}).find(item => item.category === category && item.pair === code && item.club === clubId && Number(item.playerIndex) === playerIndex); }
  function missingCheckinPlayers(match) {
    return [['ocpc', match.a], ['rebels', match.b]].flatMap(([clubId, code]) => { const pair = pairData(match.category, code); return [pair.player1, pair.player2].map((name, playerIndex) => name && !checkinRecord(match.category, code, clubId, playerIndex) ? name : '').filter(Boolean); });
  }
  function checkinWarning(match) { const missing = missingCheckinPlayers(match); return missing.length ? `Check-in needed: ${missing.join(' / ')}` : ''; }
  ...
  function scheduleCellMarkup(courtNo, minute) {
    const matches = (state.courtSchedules[courtNo] || []).map(id => state.matches.find(item => item.id === id)).filter(match => match && Number(match.startMinutes) === minute), match = matches[0], id = match?.id, active = Object.values(state.courts).some(court => court.matchId === id), done = id && isComplete(id), conflicts = id ? conflictsFor(id, courtNo) : [], checkin = match ? checkinWarning(match) : '';
  ...
  function calendarPairLine(match, side, active) { const code = side === 'a' ? match.a : match.b, pair = pairData(match.category, code), names = match.stage === 'medal' && !code ? ['TBA'] : [pair.player1, pair.player2].filter(Boolean), live = state.liveScoring?.[match.id], showServer = active && !!state.refereeAssignments?.[match.id] && live?.serving === side, serverIndex = Number(live?.serverPlayer) || 0,shown=(names.length ? names : ['Players not assigned']).map((name, index) => `${showServer && index === serverIndex ? '<i class="serving-ball" title="Currently serving"></i>' : ''}${esc(name)}`).join(pair.teamName?' & ':' / '),club=clubLabel(side==='a'?'ocpc':'rebels'); return `<b>${pair.teamName?`${esc(pair.teamName)} (`:''}${shown}${pair.teamName?')':''}<i class="timeline-affiliation">${esc(club)}</i></b>`; }
  ...
  function activeCourtMarkup(courtNo) {
    ...
    if (!match) { const nextMatch = next ? state.matches.find(item => item.id === next) : null, checkin = nextMatch ? checkinWarning(nextMatch) : ''; return `<section class="active-court empty ${checkin ? 'needs-checkin' : ''}"><b>Court vacant</b><small>${next ? `${next} is next at ${plannedLabel(courtNo, next).split('-')[0]}` : 'Schedule complete'}</small>${checkin ? `<strong class="checkin-alert">⚠ ${esc(checkin)}</strong>` : ''}${next ? `<button class="btn btn-primary" data-promote-court="${courtNo}" title="${checkin ? esc(checkin) : 'Promote the next scheduled match'}">Promote next match</button>` : ''}</section>`; }
    const live = state.liveScoring?.[match.id] || court, rules = rulesFor(match.id), seconds = elapsedSeconds(live), done = isComplete(match.id), winner = done ? (Number(scoreFor(match.id).a) > Number(scoreFor(match.id).b) ? 'a' : 'b') : '', interruption = live.interruption?.active ? live.interruption : null, interruptionSeconds = interruption ? elapsedSeconds(interruption) : 0, interruptionExpired = interruption ? interruptionHasExpired(interruption) : false, interruptionTone = interruptionExpired ? 'interruption-expired' : interruption && ['medical','technical'].includes(interruption.type) ? 'interruption-danger' : interruption && interruption.type.startsWith('timeout-') ? 'interruption-warning' : interruption ? 'interruption-danger' : '', status = courtStatusText(live, done, seconds), checkin = !done ? checkinWarning(match) : '';
    const official = state.refereeAssignments?.[match.id];
    return `<section class="active-court ${categoryClass(match.category)} ${timerClass(seconds)} ${interruptionTone} ${checkin ? 'needs-checkin' : ''}" data-active-court="${courtNo}"><div class="active-court-top"><span>${match.id} · ${esc(match.category)}</span><b data-court-status="${courtNo}">${status}</b></div>${checkin ? `<div class="checkin-alert">⚠ ${esc(checkin)}</div>` : ''}${interruption ? `<div class="court-interruption ${interruptionExpired ? 'is-expired' : ''}"><span>${interruptionExpired ? 'TIME LIMIT REACHED' : esc(interruption.label)}</span><b data-interruption-clock="${courtNo}">${clockText(interruptionSeconds)}</b><button data-end-interruption="${courtNo}">${interruptionExpired ? 'Resume Match' : 'End interruption'}</button></div>` : ''}<div class="active-score"><div class="active-team ${winner === 'a' ? 'winner' : ''}"><div>${playerPortraits(match, 'a')}</div><strong>${winner === 'a' ? '🏆 ' : ''}${esc(pairNames(match.category, match.a))}</strong><small class="pair-affiliation">${esc(clubLabel('ocpc'))}</small></div><b>${done ? scoreFor(match.id).a : live.a ?? 0}<small>–</small>${done ? scoreFor(match.id).b : live.b ?? 0}</b><div class="active-team ${winner === 'b' ? 'winner' : ''}"><div>${playerPortraits(match, 'b')}</div><strong>${winner === 'b' ? '🏆 ' : ''}${esc(pairNames(match.category, match.b))}</strong><small class="pair-affiliation">${esc(clubLabel('rebels'))}</small></div></div><div class="court-official"><span>Referee</span><b>${official ? esc(refereeName(official)) : 'Unassigned'}</b><button data-assign-official="${match.id}" data-assignment-court="${courtNo}">${official ? 'Swap' : 'Assign'}</button></div><div class="court-rule-summary"><b>${esc(rules.label)}</b><span>${rules.scoring === 'rally' ? 'Rally scoring' : 'Side-out'} · to ${rules.target} · ${rules.timer ? 'timed' : 'no timer'}</span></div><div class="active-actions">${rules.timer ? `<button class="court-timer-control" data-timer-toggle="${courtNo}"><span>${live.running ? 'Pause' : seconds ? 'Resume' : 'Start'}</span><b data-court-clock="${courtNo}">${timerText(seconds)}</b></button>` : '<span class="no-timer">No match timer</span>'}<button data-score-id="${match.id}">${done ? 'Review score' : 'Update Score'}</button>${done ? '' : `<button class="end-match-action" data-end-match="${match.id}">End Match</button>`}<button data-court-settings="${courtNo}">Scoring settings</button><button data-court-tools="${courtNo}">Match tools</button><button class="vacate" data-vacate-court="${courtNo}">${done ? 'Confirm court vacant' : 'Remove from court'}</button></div></section>`;
  }
  function interruptionLimit(interruption) { return String(interruption?.type || '').startsWith('timeout-') ? 60 : interruption?.type === 'medical' ? 300 : 0; }
  function interruptionHasExpired(interruption) { const limit = interruptionLimit(interruption); return Boolean(limit && elapsedSeconds(interruption) >= limit); }
  ...
  function showRefereeAssignment(matchId, courtNo) {
    ...
    $('#saveRefereeAssignment').onclick = () => { const email = $('#assignmentRefereeEmail').value.trim().toLowerCase(), prior = state.refereeAssignments?.[matchId]; if (!email) return toast('Choose an available referee.'); state.refereeAssignments[matchId] = email; if (prior && prior !== email && matchHasStarted(matchId)) { state.refereeHandoffs ||= []; state.refereeHandoffs.unshift({ matchId, from:prior, to:email, score:structuredClone(state.liveScoring?.[matchId]||{}), at:new Date().toISOString() }); recordActivity(`Referee handoff: ${refereeName(prior)} → ${refereeName(email)} on ${matchId}`, matchId, 'referee'); } else recordActivity(`${refereeName(email)} assigned to ${matchId}`, matchId, 'referee'); saveState(); close(); renderAll(); toast(prior && prior !== email && matchHasStarted(matchId) ? 'Referee handoff saved with live score state.' : `${refereeName(email)} assigned to ${match.id}.`); };
    if ($('#removeRefereeAssignment')) { if (matchHasStarted(matchId)) $('#removeRefereeAssignment').remove(); else $('#removeRefereeAssignment').onclick = () => { delete state.refereeAssignments[matchId]; recordActivity(`Referee removed from ${matchId}`, matchId, 'referee'); saveState(); close(); renderAll(); toast(`Referee removed from ${match.id}.`); }; }
  }
  ...
  function showCourtTools(courtNo) {
    ...
    const expired = live.interruption?.active && interruptionHasExpired(live.interruption), current = live.interruption?.active ? `<div class="tools-current ${expired ? 'is-expired' : ''}"><span>${expired ? 'Time limit reached' : 'Active now'}</span><b>${esc(live.interruption.label)}</b><strong>${clockText(elapsedSeconds(live.interruption))}</strong><button class="btn btn-primary" data-end-interruption="${courtNo}">${expired ? 'Resume Match' : 'End interruption'}</button></div>` : '';
    ...
    document.body.insertAdjacentHTML('beforeend', `<div class="court-tools-backdrop" id="courtToolsModal"><section><button class="modal-close" data-close-tools>×</button><span class="section-label">Court ${courtNo} · ${match.id}</span><h2>Match interruptions</h2><p>Choose an interruption, review it in this panel, then confirm. ${timeoutHelp}</p>${current}<div class="guarded-tools"><button data-guarded-tool="timeout-a" ${!teamTimeoutsAllowed || (live.timeouts?.a || 0) >= 1 ? 'disabled' : ''}>${esc(clubLabel('ocpc',true))} timeout <b>${teamTimeoutsAllowed ? `${live.timeouts?.a || 0}/1 · 60 sec` : 'Not allowed'}</b></button><button data-guarded-tool="timeout-b" ${!teamTimeoutsAllowed || (live.timeouts?.b || 0) >= 1 ? 'disabled' : ''}>${esc(clubLabel('rebels',true))} timeout <b>${teamTimeoutsAllowed ? `${live.timeouts?.b || 0}/1 · 60 sec` : 'Not allowed'}</b></button><button data-guarded-tool="medical">Medical timeout <b>${live.medical || 0} · 5 min max</b></button><button data-guarded-tool="technical">Technical timeout <b>${live.technicalTimeouts || 0}</b></button><button data-guarded-tool="referee">Referee timeout <b>${live.refereeTimeouts || 0}</b></button><button data-guarded-tool="equipment">Equipment timeout <b>${live.equipmentTimeouts || 0}</b></button></div><div id="toolConfirmation"></div></section></div>`);
  ...
  function armGuardedTool(courtNo, type) { const labels = { 'timeout-a': `${clubLabel('ocpc',true)} timeout`, 'timeout-b': `${clubLabel('rebels',true)} timeout`, medical: 'Medical timeout', technical: 'Technical timeout', referee: 'Referee timeout', equipment: 'Equipment timeout' }, panel = $('#toolConfirmation'); panel.innerHTML = `<div class="tool-confirm"><span>Confirm interruption</span><b>${esc(labels[type])}</b><p>The interruption timer begins immediately after confirmation.${type === 'medical' ? ' Medical timeout automatically ends at the five-minute maximum.' : ''}</p><div><button class="btn btn-primary" id="confirmGuardedTool">Confirm and start timer</button><button class="btn btn-quiet" id="cancelGuardedTool">Cancel</button></div></div>`; $('#confirmGuardedTool').onclick = () => applyGuardedTool(courtNo, type); $('#cancelGuardedTool').onclick = () => panel.innerHTML = ''; }
  function applyGuardedTool(courtNo, type) {
    const id = state.courts[courtNo]?.matchId, live = state.liveScoring[id]; if (!id || !live) return; const labels = { 'timeout-a': `${clubLabel('ocpc',true)} timeout`, 'timeout-b': `${clubLabel('rebels',true)} timeout`, medical: 'medical timeout', technical: 'technical timeout', referee: 'referee timeout', equipment: 'equipment timeout' }, label = labels[type];
  ...
  function fillCourts() { Object.keys(state.courts).forEach(no => { if (!state.courts[no].matchId) { const id = nextForCourt(Number(no)); if (id) state.courts[no] = { matchId: id, running: false, startedAt: null, elapsed: 0, calledAt: new Date().toISOString() }; } }); state.queue = state.queue.filter(id => !Object.values(state.courts).some(c => c.matchId === id)); saveState(); renderAll(); }
  ...
  function renderStandings() {
    ...
    $('#standingsGrid').innerHTML = config.clubs.map(club => {
      ...
      const method = qualificationSettings().order.map((criterion, index) => `${index + 1}. ${qualificationLabels[criterion]}`).join(' · ');
      return `<article class="standing-card"><div class="card-title"><h2>${esc(club.name)}</h2><span>Top 2 advance</span></div><div class="standing-method">${esc(method)}</div><div class="standing-row header"><span>#</span><span>Pair</span><span class="num">P</span><span class="num">W-L</span><span class="num">PF</span><span class="num">PA</span><span class="num">+/-</span></div>${rows.map(r => `<div class="standing-row"><span class="rank">${r.rank}</span><span class="team-name"><b>${r.code}</b><small>${esc(r.names)}</small></span><span class="num">${r.played}</span><span class="num">${r.wins}-${r.losses}</span><span class="num">${r.pointsFor}</span><span class="num">${r.pointsAgainst}</span><span class="num">${r.diff > 0 ? '+' : ''}${r.diff}</span></div>`).join('')}</article>`;
    }).join('');
    $$('[data-standing-category]').forEach(btn => btn.onclick = () => { standingsCategory = btn.dataset.standingCategory; renderStandings(); });
    $$('.standing-card').forEach(card => { $$('.team-name', card).forEach(node => { node.classList.add('profile-link'); node.title = 'Open live pair profile'; node.onclick = () => openPlayerProfile(standingsCategory, node.querySelector('b')?.textContent?.trim() || ''); }); });
    renderPublicShare();
  }
  function publicProjection() {
    const courts = Object.fromEntries(Object.entries(state.courts || {}).map(([courtNo, court]) => {
      const live = state.liveScoring?.[court.matchId] || {};
    ...
    const scores = Object.fromEntries(Object.entries(state.scores || {}).map(([matchId, score]) => [matchId, { a:score.a, b:score.b, completedAt:score.completedAt || null, revised:Boolean(score.revised), resultType:score.resultType || score.administrative || '' }]));
  ...
  function renderClubStandings() {
    ...
    $('#clubStandings').innerHTML = `<section class="club-scoreboard club-scoreboard-expanded"><div class="club-scoreboard-title"><div><span class="section-label">Club championship</span><h2>Overall club standings</h2><p>Every completed match contributes one win to the club total. Dream Breaker points are included only when enabled.</p></div><strong>${tied ? 'Currently tied' : `${esc(leader.short)} leads`}</strong></div><div class="club-rank-grid">${totals.map((club, index) => `<article class="club-rank-card club-dashboard-card ${index === 0 && !tied ? 'leading' : ''}"><header><span>${index + 1}</span>${clubLogoMarkup(club.id)}<div><small>${index === 0 && !tied ? 'Current leader' : 'Club standing'}</small><h3>${esc(club.name)}</h3></div><b>${club.total}<small>club points</small></b></header><div class="club-stat-grid"><span><b>${club.wins}-${club.losses}</b><small>Win-loss</small></span><span><b>${Math.round(club.winRate * 100)}%</b><small>Win rate</small></span><span><b>${club.pointsFor}</b><small>Points for</small></span><span><b>${club.pointsAgainst}</b><small>Points against</small></span><span><b class="${club.differential >= 0 ? 'positive' : 'negative'}">${club.differential > 0 ? '+' : ''}${club.differential}</b><small>Point diff.</small></span><span><b>${club.dreamPoints}</b><small>Dream Breaker</small></span></div><section class="club-recent"><h4>Recent winning contributors</h4>${club.recentWins.length ? club.recentWins.map(win => `<div><span><b>${esc(win.names)}</b><small>${esc(win.category)} · defeated ${esc(win.opponent)}</small></span><strong>${win.score}</strong></div>`).join('') : '<p>No completed wins yet.</p>'}</section></article>`).join('')}</div><p class="club-scoring-note">Club points: match wins${state.dreamBreaker?.enabled ? ' plus current Dream Breaker points' : ''}. The highest total earns Champion Club.</p></section>`;
  ...
  function renderTeams() {
    ...
    $('#teamGrid').innerHTML = config.clubs.map(club => { const count = pairCount(teamCategory, club.id); return `<article class="team-card"><div class="card-title"><h2>${esc(club.name)}</h2><span>${count} pairs</span></div>${Array.from({ length: count }, (_, i) => {
      ...
      return `<div class="team-row"><label>${code}${pair.teamName?`<small>${esc(pair.teamName)}</small>`:''}</label><input value="${esc(pair.player1)}" placeholder="Awaiting registration" readonly><input value="${esc(pair.player2)}" placeholder="Awaiting registration" readonly></div>`;
    }).join('')}</article>`; }).join('');
  ...
  function medalSeedInfo(category, value) {
    ...
    if (!raw) return { code: '', names: '' };
    if (direct) return { code: raw, names: pairNames(category, raw) };
    const match = Object.entries(state.pairs).find(([key, pair]) => {
      if (!key.startsWith(`${category}|`)) return false;
      const names = [pair.player1, pair.player2].filter(Boolean).join(' / ').trim().toLowerCase(), query = raw.toLowerCase();
      return names === query || [pair.player1, pair.player2].some(name => String(name || '').trim().toLowerCase() === query);
    });
    if (!match) return { code: raw, names: '' };
    const code = match[0].slice(`${category}|`.length);
    return { code, names: pairNames(category, code) };
  }
  function normalizeMedalSeed(category, value) { return medalSeedInfo(category, value).code; }
  function medalSeedOptions(category) {
    return Object.entries(state.pairs).filter(([key]) => key.startsWith(`${category}|`)).map(([key, pair]) => {
      const code = key.slice(`${category}|`.length), names = [pair.player1, pair.player2].filter(Boolean).join(' / ');
      return `<option value="${esc(code)}"${names ? ` label="${esc(names)}"` : ''}></option>${names ? `<option value="${esc(names)}" label="${esc(code)}"></option>` : ''}`;
    }).join('');
  }
  ...
  function syncMedalCalendar() {
    ...
    const roundRobinReady = allRoundRobinComplete();
    config.categories.forEach((category, index) => {
      const medal = state.medals[category];
      const court = [1, 3, 5][index];
      if (!court || court > config.event.courts) return;
      const seeded = Boolean(medal?.seeded && medal.bronze?.a && medal.bronze?.b && medal.final?.a && medal.final?.b);
      const bronze = seeded ? medal.bronze : { a: '', b: '' }, final = seeded ? medal.final : { a: '', b: '' };
      [['bronze', bronze, baseStart], ['final', final, baseStart + config.event.slotMinutes]].forEach(([key, sides, startMinutes]) => {
    ...
    state.queue = [...new Set([...(state.queue || []), ...state.matches.filter(match => match.stage === 'medal' && match.a && match.b && !isComplete(match.id)).map(match => match.id)])];
  ...
  function bracketMatch(category, key, label, time, court, a, b, scores) {
    ...
    const side = (code, score, scoreKey, seedKey) => { const seed = medalSeedInfo(category, code), subtitle = seed.names ? `<small>${esc(seed.names)}</small>` : ''; return `<label class="bracket-side ${result.winner === code && code ? 'winner' : ''}"><span><b><input class="seed-input" type="text" list="${medalSeedListId(category)}" data-medal-category="${esc(category)}" data-medal-match="${key}" data-medal-side="${seedKey}" value="${esc(code || '')}" placeholder="Pair code or player name" aria-label="${esc(label)} ${seedKey === 'a' ? 'first' : 'second'} pair"></b>${subtitle}</span><input type="number" min="0" max="19" data-medal-category="${esc(category)}" data-medal-match="${key}" data-medal-side="${scoreKey}" value="${score}"></label>`; };
  ...
  function renderOpponentDraw() {
    ...
    $('#drawRecommendation').innerHTML = needed ? `<section class="draw-advice recommended"><b>Opponent draw recommended</b><span>The selected cap omits at least one possible opponent in ${config.categories.filter(category => categoryNeedsDraw(category)).map(esc).join(', ')}. A locked draw provides a transparent record of those exclusions.</span></section>` : `<section class="draw-advice not-needed"><b>No opponent draw needed</b><span>Every pair plays every opposing pair in its category. There are no exclusions to randomize.</span></section>`;
    if (!needed) { panel.innerHTML = `<div class="draw-empty-state"><span class="section-label">Full round robin</span><h3>All cross-club matchups are included</h3><p>The schedule contains ${totalMatchCount()} round-robin matches. Changing pair counts keeps the full cross-club matrix automatically.</p></div>`; return; }
  ...
  function drawOpponentClub(category) { return drawSourceClub(category) === 'ocpc' ? 'rebels' : 'ocpc'; }
  function drawPairOpponents(category, draft, code) { return categoryDrawMatches(category, draft).filter(match => match.a === code || match.b === code).map(match => code.startsWith('O') ? match.b : match.a); }
  ...
  function startDrawCategory(category) {
    if (!categoryNeedsDraw(category)) return showNotice('Every possible opposing pair is already included in this category.', 'Draw not needed');
    state.drawCeremonyDraft ||= newDrawCeremonyDraft();
    if (state.drawCeremonyDraft.categories[category]) return;
    state.drawCeremonyDraft.categories[category] = { ocpc: shuffledCodes('O', pairCount(category, 'ocpc')), rebels: shuffledCodes('R', pairCount(category, 'rebels')), revealed: 0, wheelPicks: {}, wheelBagUsed: [], wheelPass: 1, startedAt: new Date().toISOString() };
  ...
  function revealNextDraw(category, wheelConfirmed = false) {
    ...
    const source = drawSourceClub(category), count = pairCount(category, source); if (draft.revealed >= count) return;
    if (state.drawCeremonyDraft.mode === 'wheel' && !wheelConfirmed) return showDrawWheel(category);
    const code = draft[source][draft.revealed], opponents = drawPairOpponents(category, draft, code);
  ...
  function showDrawWheel(category) {
    const categoryDraft = state.drawCeremonyDraft?.categories?.[category]; if (!categoryDraft) return;
    const source = drawSourceClub(category), opponent = drawOpponentClub(category), sourceOrder = categoryDraft[source], opponentOrder = categoryDraft[opponent], code = sourceOrder[categoryDraft.revealed], colors = ['#4fb6ff','#b6ff3c','#ffc94d','#b783ff','#ff6c63','#54dab1','#ff8bc3'], opponentCount = pairCount(category, opponent), matchesPerPair = drawPairOpponents(category, categoryDraft, code).length, required = Math.max(0, opponentCount - matchesPerPair);
    categoryDraft.wheelPicks ||= {}; categoryDraft.wheelBagUsed ||= []; categoryDraft.wheelPass ||= 1;
    const currentPicks = categoryDraft.wheelPicks[code] ||= [];
    ...
    const withOpponentOrder = order => ({ ...categoryDraft, [opponent]:order });
    const omissionsFor = (order,pairCode) => { const played = drawPairOpponents(category,withOpponentOrder(order),pairCode); return order.filter(other=>!played.includes(other)); };
    const allPicks = candidate => ({ ...categoryDraft.wheelPicks, [code]: [...currentPicks, ...(candidate ? [candidate] : [])] });
    const supports = (order,picks) => Object.entries(picks).every(([pairCode,selected]) => selected.every(value => omissionsFor(order,pairCode).includes(value)));
    const allOrders = permutations(opponentOrder), viableCandidates = opponentOrder.filter(candidate => !currentPicks.includes(candidate) && allOrders.some(order => supports(order,allPicks(candidate))));
    let available = viableCandidates.filter(candidate => !categoryDraft.wheelBagUsed.includes(candidate));
    if (!available.length && viableCandidates.length) { categoryDraft.wheelBagUsed = []; categoryDraft.wheelPass++; available = [...viableCandidates]; }
    let wheelOrder = [...available];
    const shuffleOrder = () => { const next = [...wheelOrder], values = crypto.getRandomValues(new Uint32Array(next.length)); for (let index = next.length - 1; index > 0; index--) { const swapIndex = values[index] % (index + 1); [next[index],next[swapIndex]] = [next[swapIndex],next[index]]; } wheelOrder = next; };
    const wheelMarkup = () => { const slices = wheelOrder.length; return `<div class="draw-wheel" id="drawWheel" style="background:conic-gradient(${wheelOrder.map((_,i)=>`${colors[i%colors.length]} ${i/slices*100}% ${(i+1)/slices*100}%`).join(',')})">${wheelOrder.map((pairCode,i)=>`<span style="--angle:${i*360/slices+180/slices}deg"><b>${esc(pairNames(category,pairCode))}</b><small>${esc(displayPair(category,pairCode))}</small></span>`).join('')}</div>`; };
    const pickNumber = currentPicks.length + 1, forced = wheelOrder.length === 1;
    $('#drawWheelOverlay')?.remove(); document.body.insertAdjacentHTML('beforeend', `<div class="draw-wheel-overlay" id="drawWheelOverlay"><section><button class="modal-close contrast-close" id="closeDrawWheel">×</button><span class="section-label">${esc(category)} · ${esc(displayPair(category,code))} · Exclusion ${pickNumber} of ${required}</span><h2>${forced ? 'Final remaining pair' : 'Spin for excluded opponent'}</h2><p>Draw bag ${categoryDraft.wheelPass} · ${wheelOrder.length} eligible pair${wheelOrder.length === 1 ? '' : 's'} remaining. Selected pairs leave this wheel until the bag is exhausted.</p><div class="draw-wheel-wrap" id="drawWheelWrap"><i class="wheel-pointer"></i>${wheelMarkup()}</div><div class="wheel-controls"><button class="btn btn-quiet" id="shuffleDrawWheel" ${forced ? 'hidden' : ''}>Shuffle names</button><button class="btn btn-primary wheel-spin-button" id="spinDrawWheel">${forced ? 'Assign final remaining pair' : 'Spin the wheel · 15 seconds'}</button></div><div class="wheel-result" id="wheelResult" hidden></div></section></div>`);
    const overlay=$('#drawWheelOverlay'); $('#closeDrawWheel').onclick=()=>overlay.remove(); $('#shuffleDrawWheel').onclick=()=>{ shuffleOrder(); $('#drawWheel').outerHTML=wheelMarkup(); toast('Names shuffled. The stop will still be generated only when Spin is pressed.'); }; $('#spinDrawWheel').onclick=()=>{ if (!wheelOrder.length) return toast('No valid pair remains for this draw.'); const random=crypto.getRandomValues(new Uint32Array(1))[0], selected=forced ? wheelOrder[0] : wheelOrder[random%wheelOrder.length], selectedIndex=wheelOrder.indexOf(selected), segment=360/wheelOrder.length, finalDegrees=forced ? 0 : 7200+(360-(selectedIndex*segment+segment/2)); currentPicks.push(selected); categoryDraft.wheelBagUsed.push(selected); const complete=currentPicks.length===required, valid=allOrders.filter(order=>supports(order,categoryDraft.wheelPicks)); if (complete && valid.length) categoryDraft[opponent]=valid[secureRandomIndex(valid.length)]; state.drawCeremonyDraft.log.push({action:forced?'wheel-auto-assigned':'wheel-spun',category,pair:code,selected,exclusionNumber:currentPicks.length,bagPass:categoryDraft.wheelPass,validOutcomes:valid.length,at:new Date().toISOString()}); saveState(); const wheel=$('#drawWheel'); if (!forced) { wheel.style.setProperty('--spin-degrees',`${finalDegrees}deg`); wheel.classList.add('spinning'); } $('#spinDrawWheel').disabled=true; $('#shuffleDrawWheel').disabled=true; setTimeout(()=>{ $('#wheelResult').innerHTML=`<span>${forced ? 'Automatically assigned' : 'Wheel selected'} · Not scheduled against</span><b>${esc(pairNames(category,selected))}<small>${esc(displayPair(category,selected))}</small></b><button class="btn btn-primary" id="confirmWheelResult">${complete ? 'Record assignment and continue' : `Continue to exclusion ${currentPicks.length + 1} of ${required}`}</button>`; $('#wheelResult').hidden=false; $('#confirmWheelResult').onclick=()=>{ overlay.remove(); complete ? revealNextDraw(category,true) : showDrawWheel(category); }; },forced?250:15100); };
  }
  ...
  function openOpponentDrawCeremony(selectedCategory = config.categories.find(category => categoryNeedsDraw(category)) || config.categories[0]) {
    if (!drawSuggested()) return showNotice('All possible cross-club matchups are included, so there are no exclusions to draw.', 'Opponent draw not needed');
    state.drawCeremonyDraft ||= newDrawCeremonyDraft();
    const ceremony = state.drawCeremonyDraft, category = selectedCategory, source = drawSourceClub(category), opponent = drawOpponentClub(category), sourceLabel = config.clubs.find(club => club.id === source).name, opponentLabel = config.clubs.find(club => club.id === opponent).name, count = pairCount(category, source), order = ceremony.categories[category], rounds = Math.ceil(categoryMatchCount(category) / count), revealed = order?.revealed || 0;
    ...
    const rows = order ? order[source].map((pairCode, index) => { const visible = index < revealed, played = drawPairOpponents(category, order, pairCode), omitted = order[opponent].filter(code => !played.includes(code)); return `<div class="draw-matrix-row ${visible ? 'is-revealed' : 'is-pending'}"><div>${visible ? pairChip(pairCode) : '<span class="draw-placeholder">Awaiting draw</span>'}</div>${Array.from({ length: rounds }, (_, round) => `<div class="${visible && played[round] ? 'draw-result' : ''}">${visible && played[round] ? `<small>R${round + 1}</small>${pairChip(played[round])}` : '<span class="draw-placeholder">Empty</span>'}</div>`).join('')}<div class="${visible ? 'draw-omission' : ''}">${visible ? (omitted.map(pairChip).join('') || '<span>None</span>') : '<span class="draw-placeholder">Pending</span>'}</div></div>`; }).join('') : '';
  ...
  function renderSettings() {
    ...
    $('#refereeList').innerHTML = assignedIds.length ? assignedIds.map(id => { const match = state.matches.find(item => item.id === id); if (!match) return ''; return `<div class="referee-assignment-row"><div><span>${id} · ${esc(match.category)} · ${timeLabel(match.startMinutes)} · Court ${match.court}</span><b>${esc(pairNames(match.category,match.a))} vs ${esc(pairNames(match.category,match.b))}</b><small>${esc(refereeName(state.refereeAssignments[id]))}</small></div><button class="btn btn-quiet" data-settings-referee="${id}">Change</button></div>`; }).join('') : '<div class="config-row"><span>No referees assigned yet</span></div>';
    ...
    const scheduling = scheduleSettings(); $('#scheduleCategoryFlow').value = scheduling.categoryFlow; $('#schedulePriority').value = scheduling.priority; $('#scheduleMinRest').value = scheduling.minRestSlots; $('#scheduleMaxGap').value = scheduling.targetMaxGapSlots; renderScheduleOptimizerSummary();
    [['whiteLabelEventName',config.event.name],['whiteLabelDate',config.event.date],['whiteLabelVenue',config.event.venue],['whiteLabelLocation',config.event.location],['whiteLabelOrganizer',config.brand.organizer],['whiteLabelShortName',config.brand.shortName],['whiteLabelLogo',config.brand.logo],['whiteLabelPrimary',config.brand.primary],['whiteLabelClubA',clubLabel('ocpc')],['whiteLabelClubAShort',clubLabel('ocpc',true)],['whiteLabelClubB',clubLabel('rebels')],['whiteLabelClubBShort',clubLabel('rebels',true)],['whiteLabelCourts',config.event.courts],['whiteLabelStart',timeInput(config.event.roundRobinStartMinutes)],['whiteLabelSlot',config.event.slotMinutes],['whiteLabelCategories',config.categories.join(', ')]].forEach(([id,value])=>{const input=$(`#${id}`);if(input)input.value=value||''});
    const qualification = qualificationSettings(); $('#qualificationOrder').innerHTML = qualification.order.map((criterion, index) => `<label><span>${index + 1}</span><select data-qualification-rank="${index}">${qualification.order.map(option => `<option value="${option}" ${option === criterion ? 'selected' : ''}>${esc(qualificationLabels[option])}</option>`).join('')}</select></label>`).join('');
  ...
  function scheduleQuality(matches = state.matches) {
    ...
    const firstWave = Math.min(...matches.map(match => match.wave)), first = matches.filter(match => match.wave === firstWave).sort((a,b)=>a.court-b.court).map(match => match.category);
    return { backToBack, longestGap, first };
  }
  function renderScheduleOptimizerSummary() { const target = $('#scheduleOptimizerSummary'); if (!target) return; const quality = scheduleQuality(); target.innerHTML = `<div class="optimizer-summary"><span><b>${quality.backToBack}</b> back-to-back pair assignments</span><span><b>${Math.max(0, quality.longestGap - 1) * config.event.slotMinutes} min</b> longest waiting gap</span><span><b>${quality.first.map(category => esc(category.replace(' Intermediate',' Inter'))).join(' / ')}</b> opening court pattern</span></div>`; }
  ...
  function saveQualificationOrder() {
    ...
    if (new Set(order).size !== order.length) return showNotice('Each qualification criterion can appear only once. Choose a different item for every position.', 'Duplicate tiebreak criterion');
    state.qualificationSettings = qualificationSettings({ order }); saveState(); renderStandings(); renderSettings(); toast('Qualification and tiebreak order saved.');
  }
  async function saveWhiteLabelSettings() {
    if (!canAdminTournament) return toast('Only the tournament owner or Full Match Control can change branding.');
    const date = $('#whiteLabelDate').value, primary = $('#whiteLabelPrimary').value, courts = Math.max(1, Math.min(20, Number($('#whiteLabelCourts').value) || 1)), startMinutes = inputMinutes($('#whiteLabelStart').value), slotMinutes = Math.max(5, Math.min(60, Number($('#whiteLabelSlot').value) || 15)), categories = $('#whiteLabelCategories').value.split(',').map(value=>value.trim()).filter((value,index,list)=>value&&list.indexOf(value)===index);
    if (!date || !categories.length) return showNotice('Enter a date and at least one category before saving.','Tournament setup incomplete');
  ...
  function applyMatchupFormat() {
    ...
    if (next.mode === current.mode && next.matchCap === current.matchCap) return toast('Matchup format is unchanged.');
    if (Object.values(state.courts).some(court => court.matchId) || Object.keys(state.scores).some(isComplete)) return showNotice('The matchup format cannot change after tournament play has started.', 'Format locked');
    const retainedPairs = state.pairs, retainedCheckins = state.checkins, retainedHistory = state.opponentDrawHistory || [], draw = createOpponentDraw(state.pairCounts);
  ...
  function renderAll() { const total = totalMatchCount(); $('#heroMatchCount').textContent = total; $('#scheduleMatchCount').textContent = `All ${total} matches`; renderOverview(); renderCourts(); renderSchedule(); renderStandings(); renderDreamBreakerBoard(); renderTeams(); renderMedals(); renderSettings(); renderArchiveDataTools(); renderOperationsViews(); }
  function renderOperationsViews() {
    const announcement = state.announcement || {};
    if ($('#announcementText')) { $('#announcementText').value = announcement.text || ''; $('#announcementEnabled').checked = Boolean(announcement.enabled); $('#announcementPreview').textContent = announcement.enabled && announcement.text ? announcement.text : 'No announcement is live.'; }
    if ($('#archiveStatus')) $('#archiveStatus').textContent = state.archive?.createdAt ? `Archive snapshot created ${new Date(state.archive.createdAt).toLocaleString()}.` : 'Create a shareable end-of-event snapshot once results are final.';
    if ($('#dataArchiveStatus')) $('#dataArchiveStatus').textContent = state.archive?.createdAt ? `Archive snapshot created ${new Date(state.archive.createdAt).toLocaleString()}.` : 'Create a shareable end-of-event snapshot once results are final.';
    if ($('#dataOpenArchive')) $('#dataOpenArchive').hidden = !state.archive?.createdAt;
  }
  function createArchiveSnapshot() { state.archive = { createdAt:new Date().toISOString(), event:structuredClone(config.event), clubStandings:clubTotals(), pairStandings:Object.fromEntries(config.categories.map(category=>[category, { ocpc:standingsFor(category,'ocpc'), rebels:standingsFor(category,'rebels') }])), medals:structuredClone(state.medals), scoreAudit:structuredClone(state.scoreAudit||[]) }; recordActivity('Event archive snapshot created', '', 'archive'); saveState(); renderOperationsViews(); toast('Archive snapshot saved to the live public view.'); }
  ...
  function openScore(id) {
    ...
    activeMatchId = id; const final = state.scores[id], live = state.liveScoring?.[id] || {}, score = final || { a: live.a ?? '', b: live.b ?? '' }, rules = rulesFor(id), maximum = Math.max(rules.target, rules.suddenDeathAt + 1);
    $('#scoreMeta').textContent = `${match.id} · Wave ${match.wave} · Court ${match.court} · ${match.category}`;
    $('#scoreTitle').textContent = match.time; $('#sideALabel').textContent = `${clubLabel('ocpc',true)} · ${match.a}`; $('#sideBLabel').textContent = `${clubLabel('rebels',true)} · ${match.b}`;
    $('#sideAName').textContent = pairNames(match.category, match.a); $('#sideBName').textContent = pairNames(match.category, match.b);
    $('#scoreA').max = maximum; $('#scoreB').max = maximum; $('#scoreA').value = score.a; $('#scoreB').value = score.b; $('#saveScoreBtn').textContent = 'Update Score'; $('#saveScoreBtn').hidden = Boolean(final);
    let endButton = $('#endMatchBtn'); if (!endButton) { $('#saveScoreBtn').insertAdjacentHTML('afterend', '<button class="btn btn-danger" id="endMatchBtn">End Match</button>'); endButton = $('#endMatchBtn'); }
    endButton.hidden = Boolean(final); endButton.onclick = () => requestEndMatch(id);
    $('#scoreMessage').textContent = final ? `Final result: ${final.a}-${final.b}. Clear it first if the match must be reopened.` : `Update Score keeps the match live. End Match accepts the current non-tied score, including before the timer expires.`; $('#scoreModal').hidden = false; setTimeout(() => $('#scoreA').focus(), 30);
  }
  function closeScore() { $('#scoreModal').hidden = true; activeMatchId = null; }
  function scoreInput(id, requireWinner = false) {
    const a = $('#scoreA').value, b = $('#scoreB').value;
    if (a === '' || b === '') { $('#scoreMessage').textContent = 'Enter both scores.'; return null; }
    const na = Number(a), nb = Number(b), rules = rulesFor(id), maximum = Math.max(rules.target, rules.suddenDeathAt + 1);
    if (!Number.isInteger(na) || !Number.isInteger(nb) || na < 0 || nb < 0 || Math.max(na, nb) > maximum) { $('#scoreMessage').textContent = `Use whole-number scores from 0 to ${maximum}.`; return null; }
    if (requireWinner && na === nb) { $('#scoreMessage').textContent = 'End Match needs a winner. Log one deciding rally first.'; return null; }
    return { a: na, b: nb, rules };
  }
  function saveScore() {
    if (!activeMatchId || state.scores[activeMatchId]) return $('#scoreMessage').textContent = 'This match is already final. Clear the result before reopening it.';
  ...
  function requestEndMatch(id) {
    if (id !== activeMatchId || state.scores[id]) return;
    const values = scoreInput(id, true); if (!values) return;
    const match = state.matches.find(item => item.id === id), live = state.liveScoring?.[id] || {}, seconds = elapsedSeconds(live), timerStatus = values.rules.timer ? `${timerText(seconds)} elapsed` : 'No match timer';
    $('#endMatchGuard')?.remove(); document.body.insertAdjacentHTML('beforeend', `<div class="end-match-guard" id="endMatchGuard"><section><button class="modal-close contrast-close" data-cancel-end>×</button><span class="section-label">Guarded match completion</span><h2>End ${esc(match?.id || id)}?</h2><div class="end-match-score"><b>${values.a}</b><span>FINAL</span><b>${values.b}</b></div><p>This records the higher current score as the winner, stops the match timer, and updates standings immediately.</p><small>${esc(timerStatus)} · This action can be corrected later through Match Control audit.</small><div><button class="btn btn-danger" id="confirmEndMatch">Confirm and End Match</button><button class="btn btn-quiet" data-cancel-end>Keep Match Live</button></div></section></div>`);
    const guard = $('#endMatchGuard'), close = () => guard.remove(); guard.onclick = event => { if (event.target === guard || event.target.closest('[data-cancel-end]')) close(); }; $('#confirmEndMatch').onclick = () => finishMatch(id, values, close);
  }
  function finishMatch(id, values, closeGuard) {
  ...
  function applyPairCounts() {
    ...
    const summary = config.categories.map(category => `${category}: ${clubLabel('ocpc',true)} ${counts[category].ocpc}, ${clubLabel('rebels',true)} ${counts[category].rebels}`).join('\n');
  ...
  function profileRoles(user, profile) { const superAdmin = ['ocpc.pickleball@gmail.com','jamescastillo37@gmail.com'].includes(String(user?.email || '').toLowerCase()); return new Set([...(Array.isArray(profile?.roles) ? profile.roles : []), ...(profile?.role ? [profile.role] : []), ...(superAdmin ? ['admin','owner','tournament_admin','match_control'] : [])]); }
  ...
  function startCloud() {
    ...
    if (drawTestMode) { document.body.insertAdjacentHTML('afterbegin', '<div class="demo-mode-banner"><b>DRAW TEST MODE</b><span>Live roster reference · browser-only ceremony · nothing enters the official audit</span></div>'); $('#launchDrawTestBtn').hidden = true; activeView = 'draw'; }
    watchAuth(async user => {
      cloudUser = demoMode || drawTestMode ? null : user;
      ...
      watchControl(incoming => {
        ...
        // Never let an acknowledgement for an older whole-event write repaint
        // newer local court/schedule intent. Per-match documents remain the
        // authority for live scoring and are merged by watchMatches below.
  ...
  $('#matchupFormatMode').onchange = event => { $('#matchupCap').disabled = event.target.value === 'full'; };
  $('#applyMatchupFormat').onclick = applyMatchupFormat;
  ...
  $('#downloadEmergencyBackup').onclick = exportBackup;
  const updateOfflineMode = () => { const offline = navigator.onLine === false, banner = $('#offlineBanner'); if (banner) banner.hidden = !offline; if (offline) localStorage.setItem(`${storageKey}.offlineDirty`, '1'); else localStorage.removeItem(`${storageKey}.offlineDirty`); };
  window.addEventListener('offline', updateOfflineMode); window.addEventListener('online', () => { updateOfflineMode(); toast('Connection restored. Live changes will sync.'); saveState(); }); updateOfflineMode();
  $('#publishAnnouncement').onclick = () => { state.announcement = { enabled: $('#announcementEnabled').checked, text: $('#announcementText').value.trim().slice(0,180), updatedAt:new Date().toISOString() }; recordActivity(state.announcement.enabled ? `Announcement published: ${state.announcement.text || 'Court call'}` : 'Announcement saved as hidden', '', 'announcement'); saveState(); renderOperationsViews(); toast(state.announcement.enabled ? 'Announcement is live on the broadcast.' : 'Announcement saved but hidden.'); };
  $('#clearAnnouncement').onclick = () => { state.announcement = { enabled:false, text:'', updatedAt:new Date().toISOString() }; recordActivity('Announcement cleared', '', 'announcement'); saveState(); renderOperationsViews(); toast('Live announcement cleared.'); };
  $('#createArchive').onclick = createArchiveSnapshot; $('#openArchive').onclick = () => { if (!state.publicShare?.token) return toast('Generate the public standings link first.'); window.open(window.MATCHDAY_EVENT_URL('public/', { token:state.publicShare.token }), '_blank', 'noopener'); };
  $('#medalCategorySelect').onchange = event => { medalCategory = event.target.value; renderMedals(); };
  $('#generatePublicLink').onclick=generatePublicLink; $('#copyPublicLink').onclick=async()=>{const token=state.publicShare?.token;if(!token)return;await navigator.clipboard.writeText(publicUrl(token));toast('Public standings link copied.');};
  ...
  $('#staffAccessBtn').onclick = async () => { if (demoMode || drawTestMode || !canAdminTournament) return toast('Only Full Match Control can change staff access.'); const email = $('#staffAccessEmail').value.trim().toLowerCase(), role = $('#staffAccessRole').value; if (!email) return toast('Enter the staff member’s website account email.'); $('#staffAccessBtn').disabled = true; try { await changeTournamentStaffRole(email, role, true); $('#staffAccessEmail').value = ''; await renderTournamentStaff(); toast('Tournament access granted.'); } catch (error) { showNotice(error.message === 'NO_ACCOUNT' ? 'That account is not in the Tournament Portal directory yet. Ask the staff member to sign in at /tournament once, then try again.' : 'Only Full Match Control can change tournament staff access.', 'Access could not be granted'); } finally { $('#staffAccessBtn').disabled = false; } };
  ...
  $('#cloudSessionBtn').onclick = () => demoMode ? resetDemo() : logout();
    $('#resetBtn').onclick = async () => { if (demoMode) return resetDemo(); if (!canAdminTournament) return toast('Only Full Match Control can reset tournament data.'); if (!confirm('Reset match-day data only? Event details, branding, club logos, competition format, registrations, and locked opponent draw will remain.')) return; $('#resetBtn').disabled = true; try { if (cloudUser) await Promise.all([clearMatches(), clearCheckins()]); const drawHistory = state.opponentDrawHistory || [], retainedPairs = structuredClone(state.pairs || {}), retainedCheckins = structuredClone(state.checkins || {}), retainedFormat = structuredClone(state.formatSettings), retainedScheduling = structuredClone(state.scheduleSettings), retainedQualification = structuredClone(state.qualificationSettings), retainedConfig = structuredClone(config); state = freshState(state.pairCounts, state.opponentDraw, retainedFormat, retainedScheduling, retainedQualification); state.opponentDrawHistory = drawHistory; state.pairs = { ...state.pairs, ...retainedPairs }; state.checkins = retainedCheckins; hydrateRegisteredTeams(false); localStorage.removeItem(storageKey); saveState(); if (cloudUser) { await updateEventConfiguration(retainedConfig); await publishControl(state); } renderAll(); toast('Match-day data reset. Event setup, branding, logos, format, registrations, and draw preserved.'); } catch (_) { alert('The reset did not fully complete. Check your connection and try again.'); } finally { $('#resetBtn').disabled = false; } };
