...
(() => {
  ...
  function bindCourtControls() {
    ...
    $$('[data-assign-official]').forEach(button => { button.onclick = event => { event.preventDefault(); event.stopPropagation(); showRefereeAssignment(button.dataset.assignOfficial, Number(button.dataset.assignmentCourt)); }; button.onpointerdown = event => event.stopPropagation(); button.ondragstart = event => event.preventDefault(); });
  }
  function showRefereeAssignment(matchId, courtNo) {
    ...
    $('#refereeAssignmentModal')?.remove(); document.body.insertAdjacentHTML('beforeend', `<div class="court-tools-backdrop" id="refereeAssignmentModal"><section class="referee-assignment-modal"><button class="modal-close" data-close-referee>×</button><span class="section-label">${courtNo ? `Court ${courtNo} · ` : ''}${match.id} · ${timeLabel(match.startMinutes)}</span><h2>${current ? 'Change referee' : 'Assign referee'}</h2><div class="assignment-matchup"><div><small>OCPC</small><b>${esc(pairNames(match.category, match.a))}</b></div><span>VS</span><div><small>Rally Rebels</small><b>${esc(pairNames(match.category, match.b))}</b></div></div><label class="official-picker">Referee email<input id="assignmentRefereeEmail" type="email" list="assignmentRefereeOptions" value="${esc(current)}" placeholder="referee@email.com"><datalist id="assignmentRefereeOptions">${known.map(email => `<option value="${esc(email)}"></option>`).join('')}</datalist></label><p>The referee can sign in or create a first-time account using this exact email. Changes take effect immediately, including during a live match.</p><div class="form-actions"><button class="btn btn-primary" id="saveRefereeAssignment">${current ? 'Save referee change' : 'Assign referee'}</button>${current ? '<button class="btn btn-danger" id="removeRefereeAssignment">Remove referee</button>' : ''}</div></section></div>`);
    ...
    $('#saveRefereeAssignment').onclick = () => { const email = $('#assignmentRefereeEmail').value.trim().toLowerCase(); if (!email) return toast('Enter the referee email.'); state.refereeAssignments[matchId] = email; saveState(); close(); renderAll(); toast(`${email} assigned to ${match.id}.`); };
    if ($('#removeRefereeAssignment')) $('#removeRefereeAssignment').onclick = () => { delete state.refereeAssignments[matchId]; saveState(); close(); renderAll(); toast(`Referee removed from ${match.id}.`); };
  }
  ...
  async function renderTournamentStaff() {
    ...
    try { const staff = await listTournamentStaff(); refereeDirectory = staff.filter(person => (Array.isArray(person.roles) ? person.roles : [person.role]).includes('tournament_referee') && person.email); $('#refereeDirectory').innerHTML = refereeDirectory.map(person => `<option value="${esc(person.email)}">${esc([person.firstName, person.lastName].filter(Boolean).join(' ') || 'Referee')}</option>`).join(''); list.innerHTML = staff.length ? staff.map(person => { const roles = Array.isArray(person.roles) ? person.roles : [person.role].filter(Boolean); return `<div class="config-row"><span>${esc(person.email || `${person.firstName || ''} ${person.lastName || ''}`)}</span><b>${roles.filter(role => role.startsWith('tournament_') || role === 'match_control').map(role => ({match_control:'Match Control',tournament_registration:'Registration',tournament_checkin:'Check-In',tournament_score_desk:'Score Kiosk',tournament_referee:'Referee'}[role])).filter(Boolean).join(', ')}</b></div>`; }).join('') : '<div class="config-row"><span>No tournament staff assigned</span></div>'; }
  ...
  function newDrawCeremonyDraft() { return { id: [...crypto.getRandomValues(new Uint8Array(6))].map(value => value.toString(16).padStart(2, '0')).join('').toUpperCase(), startedAt: new Date().toISOString(), categories: {}, log: [] }; }
  ...
  function revealNextDraw(category) {
  ...
  function renderSettings() {
    ...
    $('#refereeMatch').innerHTML = state.matches.filter(m => !isComplete(m.id)).map(m => `<option value="${m.id}">${m.id} · ${timeLabel(m.startMinutes)} · ${pairNames(m.category,m.a)} vs ${pairNames(m.category,m.b)}</option>`).join('');
  ...
  $('#assignRefereeBtn').onclick = () => { const email = $('#refereeEmail').value.trim().toLowerCase(), matchId = $('#refereeMatch').value; if (!email || !matchId) return toast('Choose a match and enter a referee email.'); state.refereeAssignments[matchId] = email; $('#refereeEmail').value = ''; saveState(); renderSettings(); renderCourts(); toast('Referee assigned.'); };
