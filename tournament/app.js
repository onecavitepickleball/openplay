...
(() => {
  ...
  function showCourtTools(courtNo) {
    ...
    document.body.insertAdjacentHTML('beforeend', `<div class="court-tools-backdrop" id="courtToolsModal"><section><button class="modal-close" data-close-tools>×</button><span class="section-label">Court ${courtNo} · ${match.id}</span><h2>Match interruptions</h2><p>Choose an interruption, review it in this panel, then confirm. ${timeoutHelp}</p>${current}<div class="guarded-tools"><button data-guarded-tool="timeout-a" ${!teamTimeoutsAllowed || (live.timeouts?.a || 0) >= 1 ? 'disabled' : ''}>OCPC timeout <b>${teamTimeoutsAllowed ? `${live.timeouts?.a || 0}/1 · 60 sec` : 'Not allowed'}</b></button><button data-guarded-tool="timeout-b" ${!teamTimeoutsAllowed || (live.timeouts?.b || 0) >= 1 ? 'disabled' : ''}>Rally Rebels timeout <b>${teamTimeoutsAllowed ? `${live.timeouts?.b || 0}/1 · 60 sec` : 'Not allowed'}</b></button><button data-guarded-tool="medical">Medical timeout <b>${live.medical || 0} · 5 min max</b></button><button data-guarded-tool="technical">Technical timeout <b>${live.technicalTimeouts || 0}</b></button><button data-guarded-tool="referee">Referee timeout <b>${live.refereeTimeouts || 0}</b></button><button data-guarded-tool="equipment">Equipment timeout <b>${live.equipmentTimeouts || 0}</b></button></div><div id="toolConfirmation"></div></section></div>`);
  ...
  function profileRoles(user, profile) { const superAdmin = ['ocpc.pickleball@gmail.com','jamescastillo37@gmail.com'].includes(String(user?.email || '').toLowerCase()); return new Set([...(Array.isArray(profile?.roles) ? profile.roles : []), ...(profile?.role ? [profile.role] : []), ...(superAdmin ? ['admin','tournament_admin','match_control'] : [])]); }
  ...
  function startCloud() {
    ...
    watchAuth(async user => {
      ...
      const roles = profileRoles(user, profile), fullControl = roles.has('admin') || roles.has('tournament_admin') || roles.has('match_control') || roles.has('court_manager');
      if (!fullControl) return showStaffPortal(user, roles);
      canAdminTournament = roles.has('admin') || roles.has('tournament_admin');
  ...
  $('#launchDemoBtn').onclick = () => { localStorage.removeItem(`${config.storageKey}.demo`); window.open(`${location.pathname}?demo=1`, '_blank', 'noopener'); }; $('#resetDemoBtn').onclick = resetDemo;
  $('#launchDrawTestBtn').onclick = () => window.open(`${location.pathname}?drawtest=1`, '_blank', 'noopener');
