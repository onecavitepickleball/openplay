...
(() => {
  ...
  function startCloud() {
    if (demoMode) { document.body.insertAdjacentHTML('afterbegin', '<div class="demo-mode-banner"><b>DEMO MODE</b><span>Simulated time: <strong id="demoClockValue">10:00 AM</strong></span><button id="demoTimeBack" title="Move simulated time back 5 minutes">−5m</button><button id="demoTimeForward" title="Move simulated time forward 5 minutes">+5m</button><button id="demoTimeReset">Reset 10:00</button></div>'); $('#demoTimeBack').onclick = () => { demoClockMinutes = Math.max(0, demoClockMinutes - 5); renderCourtTimeline(); }; $('#demoTimeForward').onclick = () => { demoClockMinutes = Math.min(1439, demoClockMinutes + 5); renderCourtTimeline(); }; $('#demoTimeReset').onclick = () => { demoClockMinutes = 600; renderCourtTimeline(); }; $('#cloudSessionBtn').textContent = 'Reset demo'; $('#cloudSessionBtn').title = 'Clear and rebuild fictional demo data'; return; }
    watchAuth(user => {
      cloudUser = user;
