...
(() => {
  ...
  function lockOpponentDrawCeremony() {
    ...
    if (!draft || !config.categories.every(category => draft.categories[category]?.revealed === pairCount(category, drawSourceClub(category)))) return toast('Complete every category before locking the draw.');
