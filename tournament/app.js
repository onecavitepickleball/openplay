...
(() => {
  ...
  function categoryMatchCount(category, counts = state?.pairCounts || defaultPairCounts()) { const ocpc = pairCount(category, 'ocpc', counts), rebels = pairCount(category, 'rebels', counts); return Math.min(ocpc * rebels, ocpc * 5, rebels * 5); }
  ...
  function categoryDrawMatches(category, order, counts = state?.pairCounts || defaultPairCounts()) {
    const ocpcCount = pairCount(category, 'ocpc', counts), rebelsCount = pairCount(category, 'rebels', counts), target = categoryMatchCount(category, counts), matches = [];
    for (let round = 0; matches.length < target && round < rebelsCount; round++) for (let index = 0; index < ocpcCount && matches.length < target; index++) matches.push({ category, round: round + 1, a: order.ocpc[index], b: order.rebels[(index + round) % rebelsCount] });
    return matches;
  }
  ...
  function freshState(pairCounts = defaultPairCounts(), retainedDraw = null) {
    ...
    return { version: 10, schedulerVersion: 6, scheduleBaselineStartMinutes: config.event.roundRobinStartMinutes, pairCounts, opponentDraw, currentWave: 1, matches, queue: matches.map(m => m.id), courtSchedules: initialCourtSchedules(matches), scheduleHistory: [], courts: emptyCourts(), matchSettings: {}, scores: {}, scoreAudit: [], refereeAssignments: {}, checkins: {}, pairs: emptyPairs(pairCounts), medals: blankMedals(), dreamBreaker: blankDreamBreaker(), updatedAt: new Date().toISOString() };
  }
  ...
  function normalizeState(incoming) {
    ...
    if (incoming?.schedulerVersion !== 6) { normalized.opponentDraw = createOpponentDraw(normalized.pairCounts); const matches = generateSchedule(normalized.pairCounts, normalized.opponentDraw); normalized.matches = matches; normalized.queue = matches.map(match => match.id); normalized.courtSchedules = initialCourtSchedules(matches); normalized.courts = emptyCourts(); normalized.scores = {}; normalized.matchSettings = {}; normalized.scoreAudit = []; normalized.refereeAssignments = {}; normalized.medals = blankMedals(); normalized.dreamBreaker = blankDreamBreaker(); normalized.drawCeremonyDraft = null; normalized.schedulerVersion = 6; normalized.scheduleBaselineStartMinutes = config.event.roundRobinStartMinutes; }
    ...
    normalized.queue = [...preserved, ...missing]; normalized.version = 10;
    return normalized;
  }
  ...
  function openOpponentDrawCeremony(selectedCategory = config.categories[0]) {
    state.drawCeremonyDraft ||= newDrawCeremonyDraft();
    const ceremony = state.drawCeremonyDraft, category = selectedCategory, count = pairCount(category, 'ocpc'), order = ceremony.categories[category], rounds = Math.min(5, pairCount(category, 'rebels')), revealed = order?.revealed || 0;
  ...
  function startCloud() {
    ...
    watchAuth(user => {
      ...
      $('#cloudGate')?.remove();
      watchControl(incoming => { if (!incoming) { publishControl(state).catch(() => showCloudGate('Your account cannot initialize this event.')); return; } const formatChanged = incoming.schedulerVersion !== 6, scheduleChanged = Number(incoming.scheduleBaselineStartMinutes) !== config.event.roundRobinStartMinutes; cloudApplying = true; const localCheckins = state.checkins || {}, normalized = normalizeState(incoming); state = { ...normalized, checkins: { ...normalized.checkins, ...localCheckins }, liveScoring: formatChanged ? {} : state.liveScoring || {} }; controlReady = true; hydrateRegisteredTeams(false); localStorage.setItem(storageKey, JSON.stringify(state)); renderAll(); cloudApplying = false; if (formatChanged) clearMatches().then(() => publishControl(state)).then(() => toast('Club-specific pair counts applied. Previous test match data cleared.')).catch(() => toast('Format updated locally; cloud cleanup needs another try.')); else if (scheduleChanged) publishControl(state).then(() => toast('Tournament start moved to 10:30 AM.')).catch(() => toast('Schedule updated locally; cloud sync needs another try.')); }, () => showCloudGate('Your account does not have Match Control access.'));
