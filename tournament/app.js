...
(() => {
  ...
  function normalizeState(incoming) {
    ...
    if (incoming?.schedulerVersion !== 3 && safeToReschedule) { const matches = generateSchedule(normalized.pairCounts); normalized.matches = matches; normalized.queue = matches.map(match => match.id); normalized.courtSchedules = initialCourtSchedules(matches); normalized.courts = emptyCourts(); normalized.schedulerVersion = 3; }
  ...
  function renderCourtTimeline() {
    ...
    const needleKey = `<div class="time-needle-key"><i></i><span>Live time needle${showNeedle ? ` · ${timeLabel(minute)}` : ` · shown on ${config.event.date} during play`}</span></div>`;
    $('#courtTimeline').innerHTML = `${headers}${liveRow}${scheduleRows}${showNeedle ? `<div class="current-time-needle" style="--needle-row:${(minute - start) / config.event.slotMinutes}"><span>${timeLabel(minute)}</span></div>` : ''}${needleKey}`;
