...
(() => {
  ...
  function startDrawCategory(category) {
    ...
    const count = pairCount(category);
    state.drawCeremonyDraft.categories[category] = { ocpc: shuffledCodes('O', count), rebels: shuffledCodes('R', count), revealed: 0, startedAt: new Date().toISOString() };
  ...
  function revealNextDraw(category) {
    ...
    const count = pairCount(category); if (draft.revealed >= count) return;
    const code = draft.ocpc[draft.revealed], rounds = Math.min(5, count), opponents = Array.from({ length: rounds }, (_, round) => draft.rebels[(draft.revealed + round) % count]);
  ...
  function lockOpponentDrawCeremony() {
    ...
    if (!draft || !config.categories.every(category => draft.categories[category]?.revealed === pairCount(category))) return toast('Complete every category before locking the draw.');
  ...
  function openOpponentDrawCeremony(selectedCategory = config.categories[0]) {
    state.drawCeremonyDraft ||= newDrawCeremonyDraft();
    const ceremony = state.drawCeremonyDraft, category = selectedCategory, count = pairCount(category), order = ceremony.categories[category], rounds = Math.min(5, count), revealed = order?.revealed || 0;
    ...
    const rows = order ? order.ocpc.map((ocpc, index) => { const visible = index < revealed, played = Array.from({ length: rounds }, (_, round) => order.rebels[(index + round) % count]), omitted = order.rebels.filter(code => !played.includes(code)); return `<div class="draw-matrix-row ${visible ? 'is-revealed' : 'is-pending'}"><div>${visible ? pairChip(ocpc) : '<span class="draw-placeholder">Awaiting draw</span>'}</div>${Array.from({ length: rounds }, (_, round) => `<div class="${visible ? 'draw-result' : ''}">${visible ? `<small>R${round + 1}</small>${pairChip(played[round])}` : '<span class="draw-placeholder">?</span>'}</div>`).join('')}<div class="${visible ? 'draw-omission' : ''}">${visible ? (omitted.map(pairChip).join('') || '<span>None</span>') : '<span class="draw-placeholder">Pending</span>'}</div></div>`; }).join('') : '';
    const allComplete = config.categories.every(item => ceremony.categories[item]?.revealed === pairCount(item));
    document.body.insertAdjacentHTML('beforeend', `<div class="draw-ceremony" id="opponentDrawCeremony"><section><header><div><span class="section-label">Transparent opponent randomizer</span><h2>Opponent Draw Ceremony</h2><p>Pending Draw ID <b>${esc(ceremony.id)}</b> · nothing changes in the official schedule until this draw is locked.</p></div><button id="closeOpponentDraw" aria-label="Close">×</button></header><div class="draw-method"><b>Live ceremony workflow</b><ol><li>Select one category and start its draw.</li><li>Both club orders are shuffled once using the browser's cryptographic random generator.</li><li>Reveal one pair's five opponents with each draw.</li><li>Review all categories, then officially lock and record the result.</li></ol><p>Closing this screen keeps the ceremony draft so it can be resumed. A started category cannot be rerolled.</p></div><nav>${config.categories.map(item => { const categoryDraft = ceremony.categories[item], status = !categoryDraft ? 'Not started' : categoryDraft.revealed === pairCount(item) ? 'Complete' : `${categoryDraft.revealed}/${pairCount(item)}`; return `<button class="${item === category ? 'active' : ''}" data-draw-category="${esc(item)}">${esc(item)} <small>${status}</small></button>`; }).join('')}</nav>${order ? `<div class="draw-order"><article><span>OCPC sealed order</span><div>${order.ocpc.map((code, index) => index < revealed ? pairChip(code) : '<span class="draw-ball concealed"><b>Sealed</b><small>Awaiting draw</small></span>').join('')}</div></article><article><span>Rally Rebels sealed order</span><div>${order.rebels.map((code, index) => index < revealed ? pairChip(code) : '<span class="draw-ball concealed"><b>Sealed</b><small>Awaiting draw</small></span>').join('')}</div></article></div><div class="draw-progress"><b>${esc(category)}</b><span>${revealed} of ${count} pair assignments revealed</span><i><u style="width:${revealed / count * 100}%"></u></i></div><div class="draw-matrix" style="--draw-cols:${rounds + 2}"><div class="draw-matrix-head"><b>OCPC pair</b>${Array.from({ length: rounds }, (_, index) => `<b>Round ${index + 1}</b>`).join('')}<b>Not scheduled</b></div>${rows}</div>` : `<div class="draw-blank"><span>Category not started</span><h3>${esc(category)}</h3><p>The board begins blank. Start when both clubs are ready to witness this category's draw.</p><button class="btn btn-primary" id="startDrawCategory">Start ${esc(category)} draw</button></div>`}<footer><div>${order && revealed < count ? `<button class="btn btn-primary" id="revealNextDraw">Draw next pair assignment</button>` : order ? '<span class="draw-complete">Category draw complete</span>' : ''}</div><button class="btn ${allComplete ? 'btn-primary' : 'btn-quiet'}" id="lockOpponentDraw" ${allComplete ? '' : 'disabled'}>Officially lock and record draw</button></footer></section></div>`);
