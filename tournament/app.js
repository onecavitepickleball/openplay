...
(() => {
  ...
  function showDrawWheel(category) {
    ...
    const code = categoryDraft.ocpc[categoryDraft.revealed], played = categoryDrawMatches(category, categoryDraft).filter(match => match.a === code).map(match => match.b), omitted = categoryDraft.rebels.filter(other => !played.includes(other)), colors = ['#4fb6ff','#b6ff3c','#ffc94d','#b783ff','#ff6c63','#54dab1','#ff8bc3'];
    ...
    $('#drawWheelOverlay')?.remove(); document.body.insertAdjacentHTML('beforeend', `<div class="draw-wheel-overlay" id="drawWheelOverlay"><section><button class="modal-close contrast-close" id="closeDrawWheel">×</button><span class="section-label">${esc(category)} · ${esc(displayPair(category,code))}</span><h2>Spin for excluded opponent${omitted.length > 1 ? 's' : ''}</h2><p>${esc(pairNames(category,code))} will not be scheduled against the pair${omitted.length > 1 ? 's' : ''} selected by this wheel.</p><div class="draw-wheel-wrap" id="drawWheelWrap"><i class="wheel-pointer"></i>${wheelMarkup()}</div><div class="wheel-controls"><button class="btn btn-quiet" id="shuffleDrawWheel">Shuffle names</button><button class="btn btn-primary wheel-spin-button" id="spinDrawWheel">Spin the wheel · 15 seconds</button></div><div class="wheel-result" id="wheelResult" hidden><span>Not scheduled against</span>${omitted.map(pairCode=>`<b>${esc(pairNames(category,pairCode))}<small>${esc(displayPair(category,pairCode))}</small></b>`).join('')}<button class="btn btn-primary" id="confirmWheelResult">Record result and continue</button></div></section></div>`);
    const overlay=$('#drawWheelOverlay'); $('#closeDrawWheel').onclick=()=>overlay.remove(); $('#shuffleDrawWheel').onclick=()=>{ shuffleOrder(); $('#drawWheel').outerHTML=wheelMarkup(); toast('Names shuffled. The audited result remains unchanged.'); }; $('#spinDrawWheel').onclick=()=>{ const wheel=$('#drawWheel'); wheel.classList.add('spinning'); $('#spinDrawWheel').disabled=true; $('#shuffleDrawWheel').disabled=true; setTimeout(()=>{$('#wheelResult').hidden=false;$('#confirmWheelResult').onclick=()=>{overlay.remove();revealNextDraw(category,true)}},15100); };
  }
