...
(() => {
  ...
  watchAuth(user => {
    ...
    if (!user) { $('#loginError').textContent = ''; return; }
    $('#staffEmail').value = user.email; $('#loginError').textContent = `Signed in as ${user.email}. Verifying Registration Desk access…`;
    if (!$('#staffLogout')) { $('#staffLogin').insertAdjacentHTML('afterend', '<button class="action alt" id="staffLogout" type="button">Sign out / switch account</button>'); $('#staffLogout').onclick = logout; }
    ...
    watchRegistrations(items => { registrations = items; $('#registrationWorkspace').hidden = false; $('#loginError').textContent = `Registration Desk access confirmed for ${user.email}.`; render(); }, () => { $('#registrationWorkspace').hidden = true; $('#loginError').textContent = 'Signed in, but Registration Desk permission is missing. An administrator must grant it in Match Control → Settings.'; });
  });
})();
