...
const { watchAuth, login, logout, authorizeTournamentTool, watchControl, watchRegistrations, createRegistration, updateRegistration, deleteRegistration, publishControl } = await import(`../firebase-sync.js?v=${encodeURIComponent(revision)}`);

(() => {
  const config = window.TOURNAMENT_CONFIG, $ = selector => document.querySelector(selector), esc = value => String(value ?? '').replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  document.querySelector('.field-brand img').src = config.brand.logo; document.querySelector('.field-header>a').href = window.MATCHDAY_EVENT_URL('../control.html');
  ...
  const opaquePairCode = (value, item = {}) => { const code = String(value || '').trim(); return !code || [item.id, item.entryId, item.stableEntryId].map(id => String(id || '').trim()).includes(code) || /^[A-Za-z0-9]{20}$/.test(code); };
  const visiblePairCode = item => { const linked = Object.values(state?.pairs || {}).find(pair => pair?.registrationId === item.id), code = String(linked?.pairCode || item.pairCode || '').trim(); return opaquePairCode(code, item) ? 'Registered pair' : code; };
  ...
  function edit(id) { const item = registrations.find(row => row.id === id); if (!item) return; editingId = id; $('#formTitle').textContent = 'Edit pair'; $('#regCategory').value = item.category; $('#regClub').value = item.club; fillPairSlots(); $('#regPair').value = item.pairCode; $('#regStatus').value = item.status; ['Name','Email','Dupr'].forEach(field => { $('#p1'+field).value = item.players[0][field === 'Name' ? 'fullName' : field.toLowerCase()] || ''; $('#p2'+field).value = item.players[1][field === 'Name' ? 'fullName' : field.toLowerCase()] || ''; }); [1,2].forEach((number,index) => { $(`#p${number}Shirt`).value = item.players[index].shirtSize || 'M'; $(`#p${number}Payment`).value = item.players[index].payment || 'pending'; }); $('#cancelEdit').hidden = false; window.scrollTo({top:0,behavior:'smooth'}); }
  ...
  watchAuth(async user => {
    ...
    let access; try { access = await authorizeTournamentTool(user, ['tournament_registration']); } catch (_) {}
    if (!access?.allowed) { $('#registrationLogin').hidden = false; $('#loginError').textContent = 'Signed in, but Registration Desk permission is missing for this tournament.'; return; }
