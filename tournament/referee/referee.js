...
(() => {
  ...
  function renderScorekeeper() {
    ...
    const aPlayers = players(match.category, match.a), bPlayers = players(match.category, match.b), servingPlayers = live.serving === 'a' ? aPlayers : bPlayers, receivingPlayers = live.serving === 'a' ? bPlayers : aPlayers, final = state.scores[match.id], rules = matchRules(match);
  ...
  function snapshot(live) { return { a: live.a, b: live.b, serving: live.serving, server: live.server, serverPlayer: live.serverPlayer, basePlayer: live.basePlayer, timeouts: { ...live.timeouts }, medical: live.medical, technicalTimeouts: live.technicalTimeouts, refereeTimeouts: live.refereeTimeouts, equipmentTimeouts: live.equipmentTimeouts }; }
  ...
  function bindScorekeeper() {
    ...
    $$('[data-point]').forEach(button => button.onclick = () => transact(`${button.dataset.point === 'a' ? 'OCPC' : 'Rally Rebels'} scored`, live => { const rules = matchRules(activeMatch()), side = button.dataset.point; if (rules.scoring === 'rally') { if (live[side] < rules.suddenDeathAt + 1) live[side]++; if (live.serving !== side) { live.serving = side; live.server = 1; live.serverPlayer = 0; } } else if (live.serving === side && live[side] < rules.suddenDeathAt + 1) live[side]++; }));
    ...
    $$('[data-timeout]').forEach(button => button.onclick = () => { const side = button.dataset.timeout, label = `${side === 'a' ? 'OCPC' : 'Rally Rebels'} timeout`; if (confirm(`Record ${label}?`)) transact(label, live => { if (live.timeouts[side] < 2) live.timeouts[side]++; }); });
    $('#medicalButton').onclick = () => { if (confirm('Record a medical timeout?')) transact('Medical timeout granted (15-minute maximum)', live => live.medical++); }; $('#technicalTimeoutButton').onclick = () => { if (confirm('Record a technical timeout?')) transact('Technical timeout', live => live.technicalTimeouts = (live.technicalTimeouts || 0) + 1); }; $('#refereeTimeoutButton').onclick = () => { if (confirm('Record a referee timeout?')) transact('Referee timeout', live => live.refereeTimeouts++); }; $('#equipmentTimeoutButton').onclick = () => { if (confirm('Record an equipment timeout?')) transact('Equipment timeout', live => live.equipmentTimeouts++); }; $('#finishButton').onclick = showConfirmation;
  }
  ...
  function showConfirmation() {
    const match = activeMatch(), live = liveFor(match), rules = matchRules(match), high = Math.max(live.a, live.b), low = Math.min(live.a, live.b), valid = high >= rules.target && high <= rules.suddenDeathAt && high - low >= 2 || high === rules.suddenDeathAt + 1 && low === rules.suddenDeathAt; if (!valid) return toast(`First to ${rules.target}, win by 2; sudden death at ${rules.suddenDeathAt}-${rules.suddenDeathAt}.`); if (live.running) toggleTimer();
