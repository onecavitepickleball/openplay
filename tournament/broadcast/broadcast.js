...
(() => {
  ...
  const pair = (category, code) => { const item = pairRecord(category, code); return item.names ? [item.names] : [item.player1 || 'Player 1', item.player2 || 'Player 2']; };
  const pairDisplay = (category, code) => { const item=pairRecord(category,code),players=[item.player1,item.player2].filter(Boolean);return item.name||item.names||(item.teamName?`${item.teamName}${players.length?` (${players.join(' & ')})`:''}`:players.join(' / '))||'Players not assigned'; };
  const compactCode = (value, fallback, length) => { const words = String(value || '').toUpperCase().replace(/[^A-Z0-9]+/g, ' ').trim().split(/\s+/).filter(Boolean), source = words.length > 1 ? words.map(word => word[0]).join('') : words[0] || fallback; return source.slice(0, length) || fallback; };
  const legacyPairCode = (value, internalCode) => { const label = String(value || '').trim(); return !label || label === String(internalCode || '') || /^[A-Za-z0-9]{20}$/.test(label) || /^[A-Z]\d+$/i.test(label); };
  const pairLabel = (category, code) => {
    const record = pairRecord(category, code), supplied = record?.pairCode;
    if (!legacyPairCode(supplied, code)) return supplied;
    const affiliationKey = record.affiliationId || record.club || '', affiliationItem = affiliations().find(item => item.id === affiliationKey), affiliationCode = compactCode(affiliationItem?.short || affiliationItem?.name || record.affiliationName || affiliationKey, 'PAIR', 4), categoryCode = compactCode(category, 'DIV', 3), seen = new Set();
    const peers = Object.values(state?.pairs || {}).filter(item => item && item.category === category && (item.affiliationId || item.club || '') === affiliationKey).filter(item => { const id = item.entryId || item.registrationId || `${item.player1}|${item.player2}`; if (seen.has(id)) return false; seen.add(id); return true; }).sort((left, right) => `${left.player1 || ''} ${left.player2 || ''}`.localeCompare(`${right.player1 || ''} ${right.player2 || ''}`, undefined, { sensitivity:'base' }));
    const targetId = record.entryId || record.registrationId || `${record.player1}|${record.player2}`, position = peers.findIndex(item => (item.entryId || item.registrationId || `${item.player1}|${item.player2}`) === targetId), legacyNumber = String(supplied || '').match(/^[A-Z](\d+)$/i);
    return `${affiliationCode}-${categoryCode}-${legacyNumber ? Number(legacyNumber[1]) : Math.max(1, position + 1)}`;
  };
  ...
  const countedMatches = () => (state?.matches || []).filter(match => match.status !== 'bye');
  ...
  const categoryClass = category => ['Novice', 'Beginner'].includes(category) ? 'novice' : ['Low Intermediate', 'Intermediate'].includes(category) ? 'low' : 'high';
  ...
  const completedCount = () => countedMatches().filter(match => state.scores?.[match.id]).length;
  ...
  function photo(category, code, id, index) { const record = pairRecord(category, code), checkin = Object.values(state?.checkins || {}).find(item => Number(item.playerIndex) === index && ((record.registrationId && item.registrationId === record.registrationId) || (item.category === category && item.pair === pairLabel(category, code) && item.club === id))); return checkin?.photoURL || checkin?.photoThumb || ''; }
  ...
  function enabledCourtNumbers() { return Object.keys(state?.courts || {}).map(Number); }
  function liveCalls() { return enabledCourtNumbers().map(no => { const match = state?.matches?.find(item => item.id === state?.courts?.[no]?.matchId); if (!match || administrativeMatch(match) || state?.scores?.[match.id]) return null; return { no, match, live:state?.liveScoring?.[match.id] || state?.courts?.[no] || {} }; }).filter(Boolean); }
  ...
  function nextAvailableMode() { return !state?.standardScheduling || state.standardScheduling.dispatchMode === 'fixed-sequence'; }
  function sequenceMatches(limit = 6) { const active = new Set(Object.values(state?.courts || {}).map(item => item?.matchId).filter(Boolean)); const byId = new Map((state?.matches || []).map(match => [match.id, match])); const ordered = (state?.queue || []).map(id => byId.get(id)).filter(Boolean); const fallback = (state?.matches || []).filter(match => !ordered.some(item => item.id === match.id)).sort((a,b) => Number(a.scheduleNumber || 0) - Number(b.scheduleNumber || 0)); return [...ordered, ...fallback].filter(match => !active.has(match.id) && !state?.scores?.[match.id] && !administrativeMatch(match) && match.status !== 'bye').slice(0, limit); }
  function sequencePanel() { if (!nextAvailableMode()) return ''; const matches = sequenceMatches(6); const node = $('#broadcastSequence'); if (!node) return ''; node.hidden = false; node.innerHTML = `<header><div><span>Dispatch order</span><h2>Next matches in sequence</h2></div><small>Promoted to the next available court</small></header>${matches.length ? `<ol>${matches.map((match,index) => `<li><em>${index + 1}</em><div><b>${esc(pairDisplay(match.category, match.a))}</b><small>${esc(affiliationName(match,'a'))}</small></div><i>vs</i><div><b>${esc(pairDisplay(match.category, match.b))}</b><small>${esc(affiliationName(match,'b'))}</small></div><span>${esc(match.category)}</span></li>`).join('')}</ol>` : '<p class="sequence-empty">No playable matches are waiting in the dispatch sequence.</p>'}`; return ''; }
  function nextMatches(no) { if (nextAvailableMode()) return ''; const matches = scheduledForCourt(no); return `<section class="court-up-next"><header><b>Up next</b><span>${matches.length ? `${matches.length} queued` : 'Court clear'}</span></header>${matches.length ? matches.map((match, index) => `<div class="next-match"><time>${timeLabel(match.startMinutes)}</time><span><b>${esc(pairDisplay(match.category, match.a))}<small>${esc(affiliationName(match,'a'))}</small></b><i>vs</i><b>${esc(pairDisplay(match.category, match.b))}<small>${esc(affiliationName(match,'b'))}</small></b></span><em>${index + 1}</em></div>`).join('') : '<p>No remaining scheduled matches.</p>'}</section>`; }
  function courtCard(no) { const court = state?.courts?.[no] || {}, found = state?.matches?.find(item => item.id === court.matchId), match = found && !administrativeMatch(found) ? found : null, upNext = nextMatches(no); if (!match) return `<article class="modern-court vacant"><header><span>COURT ${no}</span><b>AVAILABLE</b><small>Ready for play</small></header><div class="vacant-message"><i>✓</i><strong>Open court</strong><small>Waiting for Match Control</small></div>${upNext}</article>`; const live = state.liveScoring?.[match.id] || court, score = state.scores?.[match.id] || live, complete = Boolean(state.scores?.[match.id]), winner = complete && Number(score?.a) !== Number(score?.b) ? Number(score.a) > Number(score.b) ? 'a' : 'b' : '', official = state.refereeAssignments?.[match.id], seconds = elapsed(live), interruption = live.interruption?.active ? String(live.interruption.type || '').toLowerCase() : '', interruptionClass = interruption === 'team' || interruption.startsWith('timeout-') ? 'broadcast-timeout-team' : interruption === 'medical' ? 'broadcast-timeout-danger' : interruption === 'technical' || interruption === 'referee' || interruption === 'equipment' ? 'broadcast-timeout-warning' : '', status = complete ? 'FINAL' : live.interruption?.active ? String(live.interruption.label || 'TIMEOUT').toUpperCase() : live.running ? 'LIVE' : seconds ? 'PAUSED' : 'READY', matchLabel = Number(match.scheduleNumber) > 0 ? `M${Number(match.scheduleNumber)}` : 'MATCH'; const side = key => { const item = affiliation(match, key),team=pairRecord(match.category,match[key]).teamName||''; return `<div class="broadcast-side ${esc(key)} ${winner === key ? 'winner' : ''}">${winner === key ? '<span class="broadcast-winner-label">WINNER</span>' : ''}<div class="club-lockup">${logo(item)}<small>${esc(affiliationName(match, key))}<i>${esc(pairLabel(match.category, match[key]))}</i>${team?`<em>${esc(team)}</em>`:''}</small></div><div class="broadcast-players">${playerMarkup(match, key, live)}</div></div>`; }; return `<article class="modern-court ${categoryClass(match.category)} status-${complete ? 'final' : live.running ? 'live' : 'ready'} ${live.running ? 'is-live' : ''} ${interruptionClass}"><header><span>COURT ${no}</span><b>${esc(status)}</b><small>${esc(match.category)} · ${matchLabel}</small></header><div class="broadcast-match-id">${official ? `OFFICIATED BY ${esc(official.split('@')[0])}` : 'PLAYER-REPORTED SCORING'}</div><section class="broadcast-versus">${side('a')}<div class="broadcast-score"><b>${score?.a ?? 0}</b><span><i>VS</i><small>${complete ? 'FINAL' : live.serving ? `${esc(affiliationName(match, live.serving))} SERVES` : 'LIVE SCORE'}</small></span><b>${score?.b ?? 0}</b></div>${side('b')}</section><footer><span>${live.interruption?.active ? esc(status) : live.serving ? `${esc(affiliationName(match, live.serving))} SERVING` : 'MATCH TIMER'}</span><b>${timer(seconds)}</b></footer>${upNext}</article>`; }
  function pairRows(category) { const rows = new Map(); (state?.matches || []).filter(match => match.category === category && !['null','no-contest'].includes(administrativeMatch(match))).forEach(match => ['a', 'b'].filter(side => !defaultSide(match, side)).forEach(side => { const code = match[side], row = rows.get(code) || { code, pairCode:pairLabel(category, code), names:pairDisplay(category, code), affiliation:affiliationName(match, side), played:0, wins:0, pointsFor:0, pointsAgainst:0 }, score = state.scores?.[match.id]; if (score) { const other = side === 'a' ? 'b' : 'a', own = Number(score[side]) || 0, against = Number(score[other]) || 0; row.played++; row.pointsFor += own; row.pointsAgainst += against; if (own > against) row.wins++; } rows.set(code, row); })); return [...rows.values()].map(row => ({ ...row, losses:row.played - row.wins, diff:row.pointsFor - row.pointsAgainst })).sort((a, b) => b.wins - a.wins || b.diff - a.diff || String(a.pairCode).localeCompare(String(b.pairCode))); }
  function bracketRows() { const source = state?.dynamicBracket || state?.bracket || state?.brackets || state?.competition?.bracket, matches = Array.isArray(source) ? source : Array.isArray(source?.matches) ? source.matches : []; return matches.map(match => { const category = match.category || match.divisionName || '', a = match.a || match.participants?.a, b = match.b || match.participants?.b, label = code => code ? pairDisplay(category, code) : 'TBD'; return `<div><b>${esc(match.label || match.round || 'Bracket')}</b><span>${esc(label(a))} vs ${esc(label(b))}</span></div>`; }).join(''); }
  function renderStandings() { const clubBoard = $('#broadcastClubStandings'), pairBoard = $('#broadcastPairStandings'), teamRows = state?.teamStandings || state?.clubStandings || []; clubBoard.hidden = standard() ? !teamRows.length : false; if (standard()) { if (teamRows.length) clubBoard.innerHTML = `<header><span>Live team standings</span><small>1 point per representative win · walkovers included</small></header><div style="--team-count:${Math.max(1, teamRows.length)}">${teamRows.map((row,index) => `<article>${logo(row)}<span><small>#${row.rank || index + 1} · ${esc(row.short || row.name)}</small><strong>${Number(row.total || row.wins || 0)}<i>pts</i></strong></span><dl><div><dt>W-L</dt><dd>${row.wins}-${row.losses}</dd></div></dl></article>`).join('')}</div>`; const brackets = bracketRows(); pairBoard.innerHTML = `<header><div><span>Live category standings</span><h2>${brackets ? 'Pools and bracket' : 'Category leaders'}</h2></div><small>Wins · Points For · Differential</small></header><div class="pair-standing-grid">${(c.categories || []).map(category => { const rows = pairRows(category).slice(0, 6); return `<article class="pair-standing-card ${categoryClass(category)}"><h3>${esc(category)}</h3>${rows.length ? rows.map((row, index) => `<div><em>${index + 1}</em><span><b>${esc(row.names)}</b><small>${esc(row.pairCode)}${row.affiliation ? ` · ${esc(row.affiliation)}` : ''} · ${row.played} played</small></span><strong>${row.wins}W<small>${row.pointsFor} PF · ${row.diff > 0 ? '+' : ''}${row.diff}</small></strong></div>`).join('') : '<p>No completed matches yet.</p>'}</article>`; }).join('')}</div>${brackets ? `<section class="pair-standing-grid"><article class="pair-standing-card"><h3>Elimination bracket</h3>${brackets}</article></section>` : ''}`; return; } const totals = new Map(); (state?.matches || []).forEach(match => { const score = state.scores?.[match.id]; if (!score) return; ['a', 'b'].forEach(side => { const id = affiliationId(match, side), row = totals.get(id) || { id, wins:0, losses:0 }, other = side === 'a' ? 'b' : 'a'; if (Number(score[side]) > Number(score[other])) row.wins++; else if (Number(score[side]) < Number(score[other])) row.losses++; totals.set(id, row); }); }); clubBoard.innerHTML = `<header><span>Live club standings</span></header><div>${[...totals.values()].map(row => { const item = affiliations().find(entry => entry.id === row.id) || { short:row.id }; return `<article>${logo(item)}<span><small>${esc(item.short || item.name)}</small><strong>${row.wins}<i>wins</i></strong></span><dl><div><dt>W-L</dt><dd>${row.wins}-${row.losses}</dd></div></dl></article>`; }).join('')}</div>`; pairBoard.innerHTML = ''; }
  function render() { if (!state) return; state.matches ||= []; state.courts ||= {}; state.queue ||= []; state.scores ||= {}; state.liveScoring ||= {}; document.body.classList.toggle('cast-mode', cast); document.body.dataset.view = view; const courts = enabledCourtNumbers(), safeCourts = courts.length ? courts : [1]; if (!courts.includes(focus)) focus = safeCourts[0]; document.body.dataset.courts = String(safeCourts.length); document.documentElement.style.setProperty('--broadcast-court-columns', String(safeCourts.length <= 3 ? safeCourts.length : safeCourts.length === 4 ? 2 : 3)); $('#focusCourt').innerHTML = safeCourts.map(no => `<option value="${no}">Court ${no}</option>`).join(''); const visible = view === 'focus' ? [focus] : safeCourts; renderAnnouncement(); renderCalls(); const calls = new Set(liveCalls().map(call => `${call.no}:${call.match.id}`)); if (receivedState && ([...calls].some(call => !seenLiveCalls.has(call)) || completedCount() > lastCompleted || announcement() !== lastAnnouncement)) playCue(); seenLiveCalls = calls; lastCompleted = completedCount(); lastAnnouncement = announcement(); receivedState = true; $('#broadcastGrid').innerHTML = visible.map(courtCard).join(''); $('#broadcastPulse').innerHTML = `<div><span>Courts active</span><b>${safeCourts.filter(no => { const match = state.matches.find(item => item.id === state.courts?.[no]?.matchId); return match && !administrativeMatch(match); }).length}/${safeCourts.length}</b></div><div><span>Matches complete</span><b>${completedCount()}/${countedMatches().length}</b></div>`; if (nextAvailableMode()) { sequencePanel(); } else { const sequence = $('#broadcastSequence'); if (sequence) sequence.hidden = true; } renderStandings(); const queued = sequenceMatches(8), ticker = $('#broadcastTicker'); if (ticker) { const label = ticker.parentElement?.querySelector('b'); if (label) label.textContent = nextAvailableMode() ? 'DISPATCH QUEUE' : 'UP NEXT'; ticker.innerHTML = queued.length ? queued.map(match => `<span><b>${esc(pairDisplay(match.category, match.a))}</b> <i>${esc(affiliationName(match,'a'))}</i> vs <b>${esc(pairDisplay(match.category, match.b))}</b> <i>${esc(affiliationName(match,'b'))}</i> · ${esc(match.category)}</span>`).join('') : '<span>No court matches waiting</span>'; } $$('[data-broadcast-view]').forEach(button => button.classList.toggle('active', button.dataset.broadcastView === view)); $('#focusCourt').value = String(focus); $('#broadcastSound').textContent = `Sound: ${soundEnabled ? 'on' : 'off'}`; }
