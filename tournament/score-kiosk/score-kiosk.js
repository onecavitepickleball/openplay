...
(() => {
  ...
  const names = (category, code) => { const pair = state?.pairs?.[`${category}|${code}`] || {},players=[pair.player1,pair.player2].filter(Boolean); return pair.name || pair.names || (pair.teamName?`${pair.teamName}${players.length?` (${players.join(' & ')})`:''}`:players.join(' / ')) || 'Players not assigned'; };
